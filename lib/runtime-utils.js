(function initRuntimeUtils(root, factory) {
  ...
  function getTimeZoneDate(now = new Date(), timezone = 'Europe/Berlin') {
    ...
    try {
      ...
      // Calculate timezone offset to preserve the original moment while adjusting display
      // The offset compensates for the difference between:
      //   - utcTimestamp: wall-clock components interpreted as UTC (e.g., 14:30 as UTC)
      //   - sourceDate.getTime(): actual UTC timestamp of the original moment
      // Example: If source is "2026-01-15 14:30 UTC" and target TZ is "America/New_York" (UTC-5):
      //   - Wall-clock in NY: 09:30
      //   - utcTimestamp = Date.UTC(2026, 0, 15, 9, 30, 0) = time when it's 09:30 UTC
      //   - offset = sourceDate.getTime() - utcTimestamp (adjusts for TZ difference)
      //   - Result: Date that displays 09:30 via .getHours()/.getMinutes()
