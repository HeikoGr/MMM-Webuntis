(function initRuntimeUtils(root, factory) {
  ...
  function getCurrentDateContext(config = {}, options = {}) {
    const defaultTimezone =
      typeof options.defaultTimezone === "string" && options.defaultTimezone
        ? options.defaultTimezone
        : "Europe/Berlin";
    const timezone =
      typeof config?.timezone === "string" && config.timezone.trim() ? config.timezone.trim() : defaultTimezone;
  ...
  /**
   * Level logger on top of MagicMirror's Log (global config.logLevel). getLevel() is the
   * module's own logLevel: unset means the global level alone, a level narrows it further.
   */
  function createLevelLogger({ prefix = "", getLevel = () => undefined, consoleRef, methods = DEFAULT_METHODS } = {}) {
    function log(level, ...args) {
      const own = String((typeof getLevel === "function" ? getLevel() : getLevel) || "").toLowerCase();
      const messageLevel = normalizeLevel(level, "info");

      if (Object.hasOwn(LEVELS, own) && LEVELS[messageLevel] > LEVELS[own]) {
        return;
      }

      const sink = consoleRef || globalThis.Log || console;
      const methodName = methods[messageLevel] || DEFAULT_METHODS[messageLevel] || "log";
      if (prefix) {
        sink[methodName](prefix, ...args);
      } else {
        sink[methodName](...args);
      }
    }
  ...
  function parseDisplayModeTokens(displayModeValue, fallbackTokens = []) {
    const raw =
      displayModeValue === undefined || displayModeValue === null ? "" : String(displayModeValue).toLowerCase().trim();
    ...
    return Array.isArray(fallbackTokens)
      ? fallbackTokens.filter((token) => typeof token === "string" && token.trim() !== "")
      : [];
  }
