...
function getBackendPluginDefaultConfig(pluginHost, pluginId, logger) {
  ...
  try {
    ...
    logger?.('warn', null, `[plugins] Failed to read default config for plugin "${pluginId}": ${error?.message || error}`);
    return {};
  }
}
  ...
  return pluginRecords(pluginHost).map((pluginDescriptor) => {
    ...
    return {
      ...
      entry: {
        ...
        styles: Array.isArray(pluginDescriptor.entryPaths.styles) ? pluginDescriptor.entryPaths.styles.map(toRelative) : [],
      },
    };
  });
}
...
function buildFetchFlags(config, pluginHost) {
  const pluginsConfig = config && typeof config === 'object' && config.plugins && typeof config.plugins === 'object' ? config.plugins : {};
