...
class ApiStatusTracker {
  ...
  recordError(sessionKey, endpoint, err) {
    ...
    records[endpoint] = { status, recordedAt: Date.now(), failureCount, lastSuccessAt: previous?.lastSuccessAt ?? null };
  }
  ...
  recordStatus(sessionKey, endpoint, status) {
    ...
    records[endpoint] = { status, recordedAt: now, failureCount, lastSuccessAt: success ? now : (previous?.lastSuccessAt ?? null) };
  }
