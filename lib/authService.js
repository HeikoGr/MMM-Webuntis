...
class AuthService {
  ...
  async getAuthFromQRCode(qrCodeUrl, options = {}) {
    ...
    if (cached && cached.expiresAt > Date.now() + 120000) {
      return {
        ...
        // Note: appData intentionally excluded from cached result - only in fresh auth
      };
    }
    ...
    if (cached && cached.cookieString && cached.server) {
      try {
        ...
        return {
          ...
          school: cached.school,
          server: cached.server,
          // Note: appData intentionally excluded - not cached, only in fresh auth
        };
      } catch (refreshError) {
    ...
    try {
      ...
      // Cache the result (exclude appData - not needed for API calls)
      this._authCache.set(effectiveCacheKey, {
        token,
  ...
  async getAuth({ school, username, password, server, options = {} }) {
    const { cacheKey, untisClient, authSession, qrCodeUrl } = options;
    ...
    if (authSession && authSession.token && authSession.cookieString) {
      ...
      if (!cached && authSession.server && authSession.cookieString) {
        try {
          this.logger('info', '[AuthService] authSession provided but no cache - refreshing token with cookies...');
          ...
          if (qrCodeUrl) {
            ...
            return await this.getAuthFromQRCode(qrCodeUrl, { cacheKey });
          }
