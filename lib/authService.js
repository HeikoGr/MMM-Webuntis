...
class AuthService {
  ...
  async getAuth({ school, username, password, server, options = {} }) {
    ...
    if (cached && cached.expiresAt > Date.now() + fiveMinutesInMs) {
  ...
  async _performAuth(effectiveCacheKey, school, username, password, server, options) {
    ...
    // If authSession is provided (from QR or existing session), use it directly (unless forceReauth)
    if (!forceReauth && authSession?.token && authSession?.cookieString) {
      // Use existing authSession without token refresh attempts
      // Any auth errors will trigger full re-authentication via onAuthError callback
    ...
    const performRestAuth = async () => {
      ...
      // Log success AFTER caching but BEFORE returning (so logs appear before API calls start using the token)
      this.logger('debug', `[AuthService] [${username}] Credential auth: Successful for ${role || 'user'}`);
  ...
  async _fetchAppData(server, cookieString, token) {
    ...
    if (appDataResp.status === 200 && appDataResp.data) {
      ...
      try {
        ...
        this.logger('warn', `[AuthService] Failed to write raw app/data dump: ${dumpErr && dumpErr.message ? dumpErr.message : dumpErr}`);
      }
  ...
  invalidateAllCachesForSession(sessionKey) {
    ...
    for (const cacheKey of this._authCache.keys()) {
      ...
      if (cacheKey.includes(sessionPrefix) || cacheKey.includes('parent:') || cacheKey.includes('qrcode:') || cacheKey.includes('user:')) {
  ...
  /**
   * Check if cached token is about to expire (< 3 minutes remaining)
   * Used for proactive token refresh before API calls
   * @param {string} cacheKey - Cache key to check
   * @returns {boolean} true if token expires in < 3 minutes or doesn't exist
   */
  isTokenExpiringSoon(cacheKey) {
    const cached = this._authCache.get(cacheKey);
    if (!cached || !cached.expiresAt) return true; // No token = needs refresh
    const remaining = cached.expiresAt - Date.now();
    return remaining < 3 * 60 * 1000; // < 3 minutes
  }
