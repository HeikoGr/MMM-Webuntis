...
class AuthService {
  constructor(options = {}) {
    ...
    this._authCache = new Map(); // cacheKey -> { token, cookieString, tenantId, schoolYearId, appData, rawAppData, personId, role, school, server, expiresAt, lastCookieValidation }
  ...
  async getAuthFromQRCode(qrCodeUrl, options = {}) {
    ...
    if (cached && cached.expiresAt > Date.now() + 120000) {
      ...
      const COOKIE_VALIDATION_INTERVAL = 5 * 60 * 1000; // 5 minutes
      const shouldValidateCookies =
        cached.cookieString &&
        cached.server &&
        (!cached.lastCookieValidation || Date.now() - cached.lastCookieValidation > COOKIE_VALIDATION_INTERVAL);
