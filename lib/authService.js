...
class AuthService {
  constructor(options = {}) {
    ...
    this._authCache = new Map(); // cacheKey -> { token, cookieString, tenantId, schoolYearId, appData, expiresAt, lastCookieValidation }
    this._pendingAuth = new Map(); // cacheKey -> Promise (for race condition protection)
    this._cookieValidationInterval = 5 * 60 * 1000; // 5 minutes - only validate cookies periodically
  }
  ...
  async getAuthFromQRCode(qrCodeUrl, options = {}) {
    ...
    if (cached && cached.expiresAt > Date.now() + 120000) {
      ...
      // Only validate cookies periodically (every 5 minutes) to maintain cache performance
      // This prevents "Session expired" errors while avoiding network calls on every access
      const needsCookieValidation =
        cached.cookieString &&
        cached.server &&
        (!cached.lastCookieValidation || Date.now() - cached.lastCookieValidation > this._cookieValidationInterval);

      if (needsCookieValidation) {
        try {
        ...
        // No cookie validation needed - return cached data
        return {
          token: cached.token,
          cookieString: cached.cookieString,
          tenantId: cached.tenantId,
    ...
    const authPromise = this._performQRAuth(effectiveCacheKey, qrCodeUrl, cached, options);
  ...
  async _performQRAuth(effectiveCacheKey, qrCodeUrl, cached, options = {}) {
    ...
    // Starting re-authentication silently
