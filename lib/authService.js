...
class AuthService {
  ...
  async _performQRAuth(effectiveCacheKey, qrCodeUrl, cached, options = {}) {
    ...
    this.logger('debug', '[AuthService] Starting full QR code re-authentication');
    const performQRAuth = async () => {
      ...
      this.logger('debug', '[AuthService] QR code re-authentication successful');
      return {
        token,
        cookieString: cookies,
        tenantId: metadata.tenantId,
    ...
    return {
      ...
      user: appData.user
        ? {
            ...
            roles: appData.user.roles || [], // Keep roles for STUDENT/LEGAL_GUARDIAN/TEACHER detection
          }
        : null,
  ...
  _extractRoleFromAppData(appData) {
    if (!appData?.user?.roles || !Array.isArray(appData.user.roles)) {
      return null;
    }

    const roles = appData.user.roles;
