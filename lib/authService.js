...
class AuthService {
  ...
  async _performQRAuth(effectiveCacheKey, qrCodeUrl, cached) {
    ...
    if (cached?.cookieString && cached?.server) {
      const refreshedToken = await tryOrDefault(
        async () => {
        ...
        null, // Return null if refresh fails (fall through to full re-auth)
        (msg) => this.logger('warn', `[AuthService] Token refresh failed, will re-authenticate: ${msg}`)
      );
    ...
    if (authSession?.token && authSession?.cookieString) {
      ...
      if (authSession.server && authSession.cookieString) {
        ...
        if (newToken) {
          ...
          // If token refresh failed and we have QR code, re-authenticate
