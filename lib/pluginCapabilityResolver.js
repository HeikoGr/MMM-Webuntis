...
function normalizeCapabilities(capabilities) {
  ...
  return Array.from(
    new Set(
      capabilities
        .filter((capability) => typeof capability === "string" && CAPABILITY_SET.has(capability))
  ...
  for (const pluginRecord of pluginRecords) {
    ...
    const instanceCapabilities =
      typeof pluginRecord.instance?.getCapabilities === "function"
        ...
        : null;
    const sourceCapabilities = Array.isArray(instanceCapabilities)
      ? instanceCapabilities
      : pluginRecord.manifest.capabilities;
