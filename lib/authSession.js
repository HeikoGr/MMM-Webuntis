 ...
 *
 * Uses sessionKey (identifier:sessionId) or carouselId as scope for browser-session isolation.
 ...
 * @returns {string} credential key (e.g., "scope::parent:user@server/school")
 */
function getCredentialKey(student, moduleConfig, sessionKey = 'default') {
  ...
  if (isParentMode && moduleConfig) {
    if (moduleConfig.qrcode) return `${scopePrefix}qrcode:${moduleConfig.qrcode}`;
