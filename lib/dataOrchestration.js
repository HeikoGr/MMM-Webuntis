 ...
 * Handles normalization, status mapping, date range calculations for fetches
 */
...
function calculateFetchRanges(student, config, baseNow, wantsGridWidget, fetchExams, fetchAbsences, logger = () => {}) {
  ...
  if (wantsGridWidget) {
    ...
    if (typeof logger === 'function') {
      logger(`[dataOrchestration] student.grid = ${JSON.stringify(student.grid)}, gridNext = ${gridNext}`);
    }
