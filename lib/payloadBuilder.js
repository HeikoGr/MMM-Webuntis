...
function buildGotDataPayload(params) {
  ...
  const payload = {
    ...
    state: {
      fetch: {
        ...
        timetable: Boolean(fetchFlags?.fetchTimetable),
  ...
  if (payload.context?.config && Array.isArray(payload.context.config.__warnings)) {
    payload.context.config.__warnings.forEach(addWarning);
  }
  ...
  if (!activeHoliday && timetable.length === 0 && fetchTimetable && timetableLookaheadDays > 0 && hasSuccessfulTimetableStatus) {
