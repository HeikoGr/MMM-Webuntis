...
/**
 * Dump payload to debug_dumps/ folder (if dumpBackendPayloads is enabled)
 * @param {Object} payload - Payload to dump
 * @param {Object} moduleConfig - Module configuration
 * @param {Object} student - Student object (for filename)
 * @param {Function} mmLog - Logging function
 * @param {Function} cleanupOldDebugDumps - Cleanup function
 */
function dumpPayload(payload, moduleConfig, student, mmLog, cleanupOldDebugDumps) {
...
function buildGotDataPayload(params) {
  ...
  const payload = {
    ...
    apiStatus, // HTTP status codes for each API endpoint
  };
  ...
  // Optional debug dump
  dumpPayload(payload, moduleConfig, student, mmLog, cleanupOldDebugDumps);
