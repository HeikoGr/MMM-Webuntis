...
class HttpClient {
  ...
  async authenticateWithQRCode(qrCodeUrl) {
    const performAuth = async () => {
      ...
      this.logger('debug', `[HttpClient] QR Auth: Cookies after /jsonrpc_intern.do: ${cookieStr.substring(0, 100)}...`);
      ...
      this.logger('debug', `[HttpClient] QR Auth: Complete cookies before /api/app/config: ${completeCookieStr.substring(0, 100)}...`);
      ...
      this.logger('debug', `[HttpClient] QR Auth: Final cookies after /api/app/config: ${finalCookieStr.substring(0, 100)}...`);
  ...
  async getBearerToken(server, cookies) {
    ...
    const performTokenRequest = async () => {
      this.logger('debug', `[HttpClient] Requesting bearer token with cookies: ${cookies.substring(0, 100)}...`);
      ...
      this.logger(
        'debug',
        `[HttpClient] Bearer token response status: ${tokenResp.status}, content-type: ${tokenResp.headers.get('content-type')}`
      );
      ...
      this.logger('debug', `[HttpClient] Bearer token response preview: ${token.substring(0, 50)}...`);
