...
class HttpClient {
  ...
  async authenticateWithQRCode(qrCodeUrl) {
    try {
      ...
      // Pad secret to minimum 16 bytes required by otplib v13
      // WebUntis QR codes may use shorter secrets (e.g., 10 bytes)
      // Base32: each character encodes 5 bits, so 26 chars = 130 bits = 16.25 bytes
      // Pad with 'A' (Base32 zero) to meet the requirement while maintaining deterministic OTP generation
      let paddedSecret = otpSecret;
      const minBase32Chars = 26; // 26 * 5 / 8 = 16.25 bytes
      ...
      // Capture client time BEFORE generating OTP to ensure consistency
      const clientTime = new Date().getTime();

      // Generate OTP token using otplib v13 async API
      // Explicitly set TOTP parameters to match WebUntis expectations:
      // - algorithm: SHA-1 (standard TOTP default)
      // - digits: 6 (standard TOTP default)
      // - period: 30 (standard TOTP default, 30-second window)
