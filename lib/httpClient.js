...
class HttpClient {
    ...
    async authenticateWithQRCode(qrCodeUrl) {
        try {
            ...
            const authResp = await axios.post(`https://${server}/WebUntis/jsonrpc_intern.do`, requestBody, {
            ...
            if (!authResp.headers['set-cookie']) {
                throw new Error("QR code authentication failed: Server didn't return a set-cookie");
            }
            ...
            const configResp = await axios.get(`https://${server}/WebUntis/api/app/config`, {
                headers: {
