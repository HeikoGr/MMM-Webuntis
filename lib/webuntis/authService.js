      ...
      if (!effectiveStudentId && ownPersonId) {
        ...
        if (isParentLogin && appData?.user?.students && Array.isArray(appData.user.students)) {
          ...
          if (children.length > 0) {
            // Auto-select: prefer deterministic title equality, otherwise use first child.
            const matchedChild = this._matchChildByStudentTitle(children, student.title) || children[0];
