...
class AuthService {
  ...
  async _performQRAuth(effectiveCacheKey, qrCodeUrl, cached, options = {}) {
    ...
    this.logger('debug', `[AuthService]${userInfo} QR Auth: Starting authentication (cached=${cached ? 'exists' : 'null'})`);
    const performQRAuth = async () => {
      ...
      this.logger('debug', `[AuthService]${userInfo} QR Auth: success (role=${role || 'unknown'}, personId=${personId ?? 'unknown'})`);
  ...
  async getAuth({ school, username, password, server, options = {} }) {
    ...
    try {
      const result = await authPromise;
      this.logger('debug', `[AuthService] REST auth: success (role=${result.role || 'unknown'}, personId=${result.personId ?? 'unknown'})`);
      return result;
    } finally {
      // Clean up pending auth after completion
      this._pendingAuth.delete(effectiveCacheKey);
  ...
  invalidateCache(cacheKey) {
    ...
    if (this._authCache.has(cacheKey)) {
      ...
      this.logger('debug', '[AuthService] Invalidating expired token cache');
      return true;
    }
    return false;
  }
  ...
  async _fetchAppData(server, cookieString, token, options = {}) {
    ...
    if (!appData) {
      this.logger(
        'warn',
        `[AuthService] app/data returned HTTP ${appDataResp.status} without a JSON body (server=${server || 'unknown'}); tenant/school-year metadata and child list are unavailable for this session`
      );
    }
   ...
   */
  async logoutAll() {
    const entries = Array.from(this._authCache.values()).filter((entry) => entry?.cookieString && entry?.server && entry?.school);
    this._authCache.clear();
