 ...
 *
 * Progression: ~1s, ~2s, ~4s, ~8s (with ±25% random variation)
 *
...
function getRetryBackoffMs(attempt) {
  // Exponential: 2^(n-1) * 1000 = 1000ms, 2000ms, 4000ms, 8000ms, 16000ms...
  const exponentialMs = 2 ** Math.max(0, attempt - 1) * API_RETRY_BASE_MS;
  const cappedMs = Math.min(exponentialMs, API_RETRY_MAX_MS);

  // Add jitter: ±25% randomization
  const jitterRange = cappedMs * API_RETRY_JITTER_RATIO;
  const jitter = (Math.random() - 0.5) * 2 * jitterRange; // Random value in [-jitterRange, +jitterRange]
