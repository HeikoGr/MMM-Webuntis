...
function isRetryableRestError(error) {
  ...
  // Rate limiting (429) and all 5xx errors (except 501 Not Implemented) are retryable
  if (status === 429 || (status >= 500 && status !== 501)) return true;

  if (NETWORK_ERROR_CODES.has(code) || name === "ABORTERROR") return true;
...
/**
 * Detect a "successful" response whose body is really WebUntis' login page.
 *
 * The classic `/WebUntis/api/*` endpoints answer a dead session with a redirect to `index.do`
 * (handled in fetchClient), but some deployments serve the login state directly with 200:
 * either `{"loginError":"","state":"LOGIN_ERROR"}` or an HTML document. Both must be treated
 * as an expired session, never as an empty result.
 *
 * @param {any} data - Parsed response body
 * @returns {boolean} True when the body is a login page / login state
 */
function isLoginPageBody(data) {
  if (typeof data === "string") {
    return /^\s*<(!doctype|html)/i.test(data);
  }
  if (data && typeof data === "object" && !Array.isArray(data)) {
    const state = String(data.state || "").toUpperCase();
    return state === "LOGIN_ERROR" || ("loginError" in data && "state" in data);
  }
  return false;
}

function createSessionExpiredError(path, status) {
  const error = new Error(`REST ${path} answered with the WebUntis login page (HTTP ${status}): session expired`);
...
function extractErrorProperties(error) {
  const statusRaw =
    error?.status ?? error?.httpStatus ?? error?.response?.status ?? error?.cause?.status ?? error?.cause?.httpStatus;
...
async function callRestAPI({
  ...
  const performRequest = async () => {
    ...
    if (isLoginPageBody(response.data)) {
      throw createSessionExpiredError(path, response.status);
    }
  ...
  if (error) {
    ...
    const isNetworkFromCode = NETWORK_ERROR_CODES.has(code) || name === "ABORTERROR";
    ...
    if (isNetworkFromCode || isNetworkFromText) {
      ...
      if (error?.body) logger("debug", `  - response body: ${JSON.stringify(String(error.body).slice(0, 200))}`);
      if (error?.location) logger("debug", `  - redirect location: ${error.location}`);
    }
