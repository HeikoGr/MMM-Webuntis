...
const { parseHHMMStringToInteger } = require('./dataOrchestration');
...
const schemas = {
  lesson: {
    ...
    statusDetail: { from: 'statusDetail', default: null },
    layoutWidth: { from: 'layoutWidth', default: 1000 }, // 1000=full-class, 500=parallel half-group (from Webuntis layout hints)
    id: { from: 'id', default: null },
    // NOTE: 'lessonId' is not always populated by webuntisApiService.getTimetable(), but is kept for compatibility with timegrid/legacy data.
    lessonId: { from: 'lessonId', default: null },
    // Previous values for CHANGED lessons – present only when a field actually changed
    teOld: {
      from: 'teOld',
      transform: (v) => (Array.isArray(v) && v[0] ? [{ name: v[0].name, longname: v[0].longname }] : []),
      default: [],
...
function sanitizeHtml(text, allowMarkdown = false) {
  ...
  // Decode common HTML entities first so encoded markup is filtered by the whitelist logic
  // instead of being reintroduced as live HTML after sanitization.
  result = result
    .replace(/&nbsp;/g, ' ')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
