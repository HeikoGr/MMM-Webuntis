...
class HttpClient {
  ...
  async authenticateWithQRCode(qrCodeUrl) {
    const performAuth = async () => {
      ...
      this._cookieJar.cookies.set(schoolnameKey, `_${Buffer.from(school).toString('base64')}`);

      // Get complete cookie string including schoolname
      const completeCookieStr = this._cookieJar.getCookieString(server);
      ...
      if (configResp.headers) {
        this._cookieJar.setCookies(configResp.headers, server);
      }

      // Get updated cookie string after app/config request
      const finalCookieStr = this._cookieJar.getCookieString(server);
      ...
      // QR authentication successful - no log needed (implicit success)
  ...
  async getBearerToken(server, cookies) {
    // Getting bearer token silently (no log for cleaner output)
    const performTokenRequest = async () => {
      ...
      // Token obtained successfully (no log needed)
      return token;
    };
