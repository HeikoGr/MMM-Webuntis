...
function createTaggedError(message, metadata = {}) {
...
class HttpClient {
  ...
  async authenticateWithQRCode(qrCodeUrl) {
    const performAuth = async () => {
      ...
      // Validate QR params without logging sensitive data
      ...
      if (otpSecret.length < minBase32Chars) {
        ...
        // Padding applied silently - no log needed
      }
      ...
      if (authResp.status !== 200 || authResp.data?.error) {
        ...
        // JSON-RPC reports rejected logins inside a 200 body; tag it so status tracking and
        // warnings treat it as an auth failure, not as a successful call.
        throw createTaggedError(`QR code authentication failed: ${authResp.status} - ${errorMsg}`, {
          code: 'AUTH_FAILED',
          isAuthError: true,
      ...
      // One jar per login: the service is shared by every module instance, and two accounts on
      // the same server logging in concurrently must never see each other's JSESSIONID.
      const cookieJar = new CookieJar();
      cookieJar.setCookies(authResp.headers, server);
      const cookieStr = cookieJar.getCookieString(server);
      const sessionId = this._extractSessionId(cookieStr);

      this.logger('debug', `[HttpClient] QR Auth: Session validation completed`);

      if (!sessionId) {
        throw new Error('QR code authentication failed: No JSESSIONID in cookies');
      }

      // Add schoolname cookie explicitly (WebUntis expects this)
      const schoolnameKey = `${server}:schoolname`;
      cookieJar.cookies.set(schoolnameKey, `_${Buffer.from(school).toString('base64')}`);

      // Get complete cookie string including schoolname
      const completeCookieStr = cookieJar.getCookieString(server);

      const configResp = await fetchClient.get(`https://${server}/WebUntis/api/app/config`, {
        headers: {
          Cookie: completeCookieStr, // Use all cookies from CookieJar
        },
        timeout: 10000,
      });

      // Update CookieJar with any new cookies from app/config response
      // This ensures we capture all session cookies (JSESSIONID, schoolname, etc.)
      if (configResp.headers) {
        cookieJar.setCookies(configResp.headers, server);
      }

      // Get updated cookie string after app/config request
      const finalCookieStr = cookieJar.getCookieString(server);
      ...
      this.logger('debug', `[HttpClient] QR login successful (school=${school}, server=${server}, personType=${personType ?? 'unknown'})`);

      return {
        cookies: finalCookieStr, // Return all cookies including ones from app/config
        sessionId,
        personId,
        personType,
        school,
    ...
    try {
      ...
      const msg = String(err?.message || err?.cause?.message || 'Unknown error');

      const isNetworkCode = ['ECONNREFUSED', 'ETIMEDOUT', 'ECONNRESET', 'EHOSTUNREACH', 'ENOTFOUND', 'EAI_AGAIN'].includes(code);
      const isNetworkText =
        ...
        msg.toLowerCase().includes('ehostunreach');

      if (isNetworkCode || isNetworkText) {
   ...
   * Used to validate session establishment and track authentication state
   *
   * @param {string} cookieString - Cookie header string (e.g., "JSESSIONID=abc123; Path=/")
   * @returns {string|null} Session ID value or null if not found
   * @private
   */
    ...
    const performAuth = async () => {
      ...
      if (authResp.status !== 200 || !authResp.data?.result) {
        throw createTaggedError(
          `Credentials authentication failed: ${authResp.status} - ${authResp.data?.error?.message || 'Unknown error'}`,
          {
            code: 'AUTH_FAILED',
            isAuthError: true,
            httpStatus: authResp.status,
      ...
      // One jar per login (see authenticateWithQRCode)
      const cookieJar = new CookieJar();
      cookieJar.setCookies(authResp.headers, server);
      const cookieStr = cookieJar.getCookieString(server);
      ...
      this.logger(
        'debug',
        `[HttpClient] Credential login successful (school=${school}, server=${server}, personType=${authResp.data.result.personType ?? 'unknown'})`
      );
  ...
  async getBearerToken(server, cookies) {
    const performTokenRequest = async () => {
      ...
      // this.logger('debug', `[HttpClient] Bearer token: ${tokenResp.status} (${tokenResp.status === 200 ? 'received' : 'failed'})`);

      if (tokenResp.status !== 200) {
        throw createTaggedError(`Bearer token request failed: HTTP ${tokenResp.status}`, {
      ...
      if (typeof tokenResp.data !== 'string' || tokenResp.data.trim().length === 0) {
        throw createTaggedError('Bearer token response is empty or invalid', {
          code: 'TOKEN_INVALID',
          isAuthError: true,
          httpStatus: tokenResp.status,
        });
      ...
      if (token.startsWith('<') || token.startsWith('{') || token.includes('<!DOCTYPE')) {
        this.logger('error', `[HttpClient] Received HTML/JSON instead of token. First 200 chars: ${token.substring(0, 200)}`);
        throw createTaggedError('Session expired: received HTML/JSON instead of bearer token', {
      ...
      this.logger('debug', `[HttpClient] Bearer token obtained (server=${server}, length=${token.length})`);
      return token;
    };
