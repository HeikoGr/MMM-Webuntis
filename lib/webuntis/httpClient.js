...
class HttpClient {
  ...
  async authenticateWithQRCode(qrCodeUrl) {
    const performAuth = async () => {
      ...
      this.logger(
        "debug",
        `[HttpClient] QR login successful (school=${school}, server=${server}, personType=${personType ?? "unknown"})`,
      );
  ...
  async getBearerToken(server, cookies) {
    const performTokenRequest = async () => {
      ...
      if (token.startsWith("<") || token.startsWith("{") || token.includes("<!DOCTYPE")) {
        this.logger(
          "error",
          `[HttpClient] Received HTML/JSON instead of token. First 200 chars: ${token.substring(0, 200)}`,
        );
