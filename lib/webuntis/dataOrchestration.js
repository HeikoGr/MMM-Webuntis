...
function calculateFetchRanges(input = {}) {
  ...
  if (wantsGridWidget) {
    ...
    const effectiveGridNext =
      !weekView && hideWeekends
        ...
        : gridNext;
    const effectiveGridPast =
      !weekView && hideWeekends ? calculateCalendarOffsetForVisibleWeekdays(baseNow, gridPast, -1) : gridPast;
  ...
  if (wantsLessonsWidget) {
    ...
    const effectiveLessonsNext = hideWeekends
      ...
      : lessonsNext;
    const effectiveLessonsPast = hideWeekends
      ? calculateCalendarOffsetForVisibleWeekdays(baseNow, lessonsPast, -1)
      : lessonsPast;
