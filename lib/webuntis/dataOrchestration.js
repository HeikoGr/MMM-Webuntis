 ...
 *
 * Keeps the tags in RICH_TEXT_ALLOWED_TAGS (b, strong, i, em, ...), drops every attribute, turns
 * block and <br> tags into newlines, and decodes entities. The result is declared safe HTML and
 * must NOT be escaped again downstream - escaping it a second time is what made users see literal
 * `&amp;` and `<b>` (see docs/API_REFERENCE.md, "HTML Sanitization").
 *
 ...
 * @returns {string} Sanitized rich text (safe HTML, already entity-decoded)
 */
function sanitizeRichText(text, keepMarkdownMarkers = false) {
  ...
  // sanitize-html re-serializes to valid HTML, so entities like &amp; survive as entities -
  // decode them into their literal characters (see stripAllHtml for the same rationale).
  result = decodeHTML(result);
...
function extractTimegridFromTimetable(timetable) {
  ...
  const sortedStarts = Array.from(startTimes).sort((a, b) => (normalizeTimeToHHMM(a) || 0) - (normalizeTimeToHHMM(b) || 0));
...
function calculateFetchRanges(input = {}) {
  ...
  if (wantsGridWidget) {
    ...
    const hideWeekends = Boolean(options.gridHideWeekends);
    ...
    const effectiveGridNext =
      !weekView && hideWeekends
        ? calculateCalendarOffsetForVisibleWeekdays(baseNow, gridNext + (isWeekendDay(baseNow) ? 1 : 0), 1)
        : gridNext;
    const effectiveGridPast = !weekView && hideWeekends ? calculateCalendarOffsetForVisibleWeekdays(baseNow, gridPast, -1) : gridPast;

    timetableNextDays = Math.max(timetableNextDays, effectiveGridNext, minNextDaysForWeekView);
    timetablePastDays = Math.max(timetablePastDays, effectiveGridPast);
  }

  if (wantsLessonsWidget) {
    const hideWeekends = Boolean(options.lessonsHideWeekends);
    const lessonsNext = Number(days.lessonsNextDays ?? 2);
    const lessonsPast = Number(days.lessonsPastDays ?? 0);
    const effectiveLessonsNext = hideWeekends
      ? calculateCalendarOffsetForVisibleWeekdays(baseNow, lessonsNext + (isWeekendDay(baseNow) ? 1 : 0), 1)
      : lessonsNext;
    const effectiveLessonsPast = hideWeekends ? calculateCalendarOffsetForVisibleWeekdays(baseNow, lessonsPast, -1) : lessonsPast;
    timetableNextDays = Math.max(timetableNextDays, effectiveLessonsNext);
    timetablePastDays = Math.max(timetablePastDays, effectiveLessonsPast);
  }
  ...
  if (validatedExamsDays < 1 || validatedExamsDays > 360 || Number.isNaN(validatedExamsDays)) {
