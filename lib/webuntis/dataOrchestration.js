...
function stripAllHtml(text, preserveLineBreaks = true) {
  ...
  // Step 4: Re-encode angle brackets to prevent re-formed tags while preserving text meaning
  result = result.replace(/</g, '&lt;').replace(/>/g, '&gt;');

  // Step 5: Clean up extra whitespace
  if (preserveLineBreaks) {
    // Collapse non-newline whitespace to single spaces, but keep '\n' as line breaks
    result = result.replace(/[^\S\n]+/g, ' ');
    // Normalize multiple consecutive newlines to a single newline
    result = result.replace(/\n{2,}/g, '\n');
    // Trim whitespace at line ends and overall
    result = result
      .split('\n')
      .map((line) => line.trim())
      .join('\n')
...
function normalizeTimeToHHMM(time) {
  ...
  // Parse strict HH:MM input first
 ...
 * @param {Object} fetchPlan - Fetch plan flags
 * @param {boolean} fetchPlan.wantsGridWidget - Whether grid widget is enabled
 * @param {boolean} fetchPlan.wantsLessonsWidget - Whether lessons widget is enabled
 * @param {boolean} fetchPlan.fetchExams - Whether exams should be fetched
 * @param {boolean} fetchPlan.fetchAbsences - Whether absences should be fetched
 * @returns {Object} Date ranges for timetable, exams, homework, absences
 */
function calculateFetchRanges(student, config, baseNow, fetchPlan = {}) {
  ...
  if (wantsLessonsWidget) {
    const lessonsNext = Number(student.lessons?.nextDays ?? 2);
    const lessonsPast = Number(student.lessons?.pastDays ?? 0);
  ...
  const examsNextDays = student.exams?.nextDays ?? 0;
  ...
  const absPast = Number.isFinite(Number(student.absences?.pastDays ?? 0)) ? Number(student.absences?.pastDays ?? 0) : 0;
  const absFuture = Number.isFinite(Number(student.absences?.nextDays ?? 0)) ? Number(student.absences?.nextDays ?? 0) : 0;
  ...
  const hwNextDays = Number(student.homework?.nextDays ?? 0);
