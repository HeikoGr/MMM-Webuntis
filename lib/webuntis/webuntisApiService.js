  ...
  try {
    ...
    if (dumpRaw) {
      try {
        ...
        const redactedData = JSON.parse(JSON.stringify(data ?? null));
        redactDumpFields(redactedData);
        fs.writeFileSync(filePath, JSON.stringify(redactedData, null, 2), { encoding: 'utf8' });
...
async function getTimetable({
  ...
  const transform = (resp) => {
    ...
    if (!resp?.days || !Array.isArray(resp.days) || resp.days.length === 0) {
