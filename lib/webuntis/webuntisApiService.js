...
const { isAuthError } = require("./errorHandler");
...
/**
 * A timetable body without a `days` array is never a legitimate "no lessons" answer: WebUntis
 * always returns one entry per requested day, each carrying a status (REGULAR, NO_DATA,
 * NOT_ALLOWED). Anything else is a transport or session problem and must surface as an error
 * so the frontend keeps its previous data instead of rendering "no lessons".
 */
function createInvalidTimetableError(resp) {
  const shape =
    resp === null || resp === undefined
      ? String(resp)
      : typeof resp === "string"
        ? `string(${resp.length}) starting with ${JSON.stringify(resp.slice(0, 40))}`
        : `${Array.isArray(resp) ? "array" : "object"} with keys ${JSON.stringify(Object.keys(resp).slice(0, 8))}`;
  const error = new Error(`timetable response has no days[] (${shape})`);
  error.code = "INVALID_RESPONSE";
  return error;
}

function writeRawDump({ server, dataType, payload, suffix = "", logger, logPrefix }) {
  try {
    const dumpDir = path.join(__dirname, "..", "..", "debug_dumps");
    fs.mkdirSync(dumpDir, { recursive: true });
    const serverSafe = String(server || "server").replace(/[^a-z0-9.-]/gi, "_");
    const filePath = path.join(dumpDir, `raw_api_${Date.now()}_${serverSafe}_${dataType}${suffix}.json`);
    const redacted = JSON.parse(JSON.stringify(payload ?? null));
    redactDumpFields(redacted);
    fs.writeFileSync(filePath, JSON.stringify(redacted, null, 2), { encoding: "utf8" });
    cleanupOldRawDumps(dumpDir);
  } catch (dumpErr) {
    logWithStudentContext(
      logger,
      "warn",
      `${logPrefix} Failed to write raw API dump: ${dumpErr?.message ? dumpErr.message : dumpErr}`,
    );
  }
}
 ...
 */
async function callWebUntisAPI({
  dataType,
  authContext,
  server,
  params,
  ...
  const cache = authContext.responseCache || null;
  ...
  const makeRequest = async () => {
    ...
    const cacheKey = cache
      ? responseCache.buildKey({ scope: cache.scope, server, path: endpoint, schoolYearId, params })
      : null;
    const cached = cacheKey ? cache.store.get(cacheKey, cache.maxAgeMs) : undefined;
    if (cached) {
      logWithStudentContext(logger, "debug", `${logPrefix} reusing a response from another session of this account`);
    ...
    if (cacheKey) cache.store.set(cacheKey, { data: response.data, status: response.status });
  ...
  try {
    ...
    if (dumpRaw && !response.fromCache) {
      writeRawDump({ server, dataType, payload: data, logger, logPrefix });
    }
    ...
    if (dumpRaw) {
      // Failed calls are the ones worth keeping when chasing sporadic outages.
      writeRawDump({
        server,
        dataType,
        suffix: "_error",
    ...
    if (isAuthError(error) && typeof authContext.onAuthError === "function") {
      logWithStudentContext(
        logger,
        "warn",
        `${logPrefix} Authentication token expired, invalidating cache and retrying...`,
      );
...
const CHANGED_FIELD_NAME_MAPPING = {
  teachers: "teacher",
  subjects: "subject",
  rooms: "room",
  classes: "class",
...
// WebUntis marks every requested day: REGULAR (may still carry no entries), NO_DATA (nothing
// scheduled - weekends, past school years) and NOT_ALLOWED (viewing this day is locked). NO_DATA
// and NOT_ALLOWED are forwarded as day notices so the frontend can tell a server-confirmed
// lesson-free day from a day for which it simply has no data.
const TIMETABLE_DAY_NOTICE_KINDS = new Map([
  ["NOT_ALLOWED", "timetable-restricted"],
  ["NO_DATA", "no-data"],
]);
const TIMETABLE_POSITION_KEYS = [
  "position1",
  "position2",
  "position3",
  "position4",
...
function buildTimetableDayNotice(day) {
  const rawStatus = String(day?.status || "")
    .trim()
    .toUpperCase();
  const kind = TIMETABLE_DAY_NOTICE_KINDS.get(rawStatus);
  if (!kind) return null;

  const rawDate = typeof day?.date === "string" ? day.date.split("T")[0] : "";
...
function pushMappedField(result, fieldKey, current) {
...
function pushRemovedField(result, fieldKey, removed) {
  if (!removed?.shortName) return;

  const oldFieldKey = `previous${fieldKey.charAt(0).toUpperCase()}${fieldKey.slice(1)}`;
  if (!(oldFieldKey in result)) return;

  result[oldFieldKey].push({
    name: removed.shortName,
    longname: removed.longName,
  });

  const changedFieldName = CHANGED_FIELD_NAME_MAPPING[fieldKey] || fieldKey;
  if (!result.changedFields.includes(changedFieldName)) {
    result.changedFields.push(changedFieldName);
  }
}

function mapPositionEntry(result, pos, posKey, logger) {
  ...
  pushMappedField(result, fieldKey, current);
  pushRemovedField(result, fieldKey, pos.removed);
}
...
function mapPositionsToFields(entry, logger) {
  ...
  TIMETABLE_POSITION_KEYS.forEach((posKey) => {
    const positions = entry[posKey];
    if (!Array.isArray(positions)) return;
    positions.forEach((pos) => {
      mapPositionEntry(result, pos, posKey, logger);
    });
  });
...
async function getTimetable({
  ...
  const transform = (resp) => {
    ...
    if (!Array.isArray(resp?.days)) {
      throw createInvalidTimetableError(resp);
    }

    for (const day of resp.days) {
      const dayNotice = buildTimetableDayNotice(day);
      if (dayNotice) {
        dayNotices.push(dayNotice);
      }
      ...
      for (const entry of entries) {
        ...
        lessons.push({
          ...
          teachers: mappedFields.teachers,
          subjects: mappedFields.subjects,
          rooms: mappedFields.rooms,
          classes: mappedFields.classes,
          studentGroups: mappedFields.studentGroups,
          info: mappedFields.info,
          previousTeachers: mappedFields.previousTeachers,
          previousSubjects: mappedFields.previousSubjects,
          previousRooms: mappedFields.previousRooms,
          changedFields: mappedFields.changedFields,
          substitutionText: entry.substitutionText || "",
          lessonText: entry.lessonInfo || "",
          status: entry.status || "REGULAR",
        });
      }
    }

    lessons.dayNotices = dayNotices;
    return lessons;
  };
  ...
  const transform = (data) => {
    ...
    examArr.forEach((exam) => {
      const isAssignedToStudent =
        ...
        exam.assignedStudents.some((s) => Number(s.id) === Number(personId));
      ...
      if (!personId || isAssignedToStudent) {
        exams.push({
          ...
          // stripHtml is stripAllHtml(text, preserveLineBreaks) - the boolean is about line
          // breaks, not about markup: exam fields arrive here as plain text and stay plain text.
          name: stripHtml ? stripHtml(exam.name ?? exam.examType ?? exam.lessonName ?? "", false) : (exam.name ?? ""),
          subject: stripHtml ? stripHtml(exam.subject ?? exam.lessonName ?? "", false) : (exam.subject ?? ""),
          teachers: Array.isArray(exam.teachers) ? exam.teachers : [],
          text: stripHtml ? stripHtml(exam.text ?? exam.description ?? "", true) : (exam.text ?? ""),
        });
      }
    });
...
async function getHomework({
  ...
  const transform = (data) => {
    ...
    hwArray.forEach((hw) => {
      ...
      } else if (Array.isArray(lesson?.subjects)) {
        subjectData = lesson.subjects;
      } else if (Array.isArray(hw?.subjects)) {
        subjectData = hw.subjects;
      }
  ...
  const transform = (data) => {
    ...
    absArr.forEach((abs) => {
      ...
      absences.push({
        ...
        subjects: abs.su?.[0]?.name ? [{ name: abs.su[0].name, longname: abs.su[0].longname ?? abs.su[0].name }] : [],
        teachers: abs.te?.[0]?.name ? [{ name: abs.te[0].name, longname: abs.te[0].longname ?? abs.te[0].name }] : [],
