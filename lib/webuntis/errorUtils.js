 ...
 * Normalizes logger signatures to a single message-only callback.
 * @private
 */
function toErrorLogger(logger) {
  if (!logger || typeof logger !== 'function') return null;

  const arity = Number(logger.length) || 0;
  if (arity >= 3) {
    return (message) => logger('error', null, message);
  }
  if (arity === 2) {
...
function logError(err, logger) {
  // Errors flagged `quiet` are expected repeats (e.g. an endpoint skipped after a 403) and
  // only carry a warning to the frontend; they do not deserve an error log per cycle.
  if (err?.quiet === true) return;

  const errorLogger = toErrorLogger(logger);
 ...
 * @param {Object} opts
 * @param {Function} opts.logger - logger function (accepted: (level, student, msg), (level, msg), or (msg))
 ...
 * Equivalent to tryOrDefault(fn, null, logger).
...
function tryOrNull(fn, logger) {
  return tryOrDefault(fn, null, logger);
}
