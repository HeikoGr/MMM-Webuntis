...
const { calculateFetchRanges, sanitizeHtmlText, normalizeDateToInteger, normalizeTimeToMinutes } = require('./dataOrchestration');
...
class WebUntisClient {
  ...
  static sanitizeHtmlText(text, preserveLineBreaks = true) {
  ...
  static normalizeTimeToMinutes(time) {
  ...
  async _getTimetableViaRest(
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, 'timetable')) {
      const prevStatus = this.getApiStatus?.(sessionKey)?.timetable;
    ...
    if (sessionKey && response.status) {
      this._setApiStatus(sessionKey, 'timetable', response.status);
    }
  ...
  async _getExamsViaRest(authCtx, sessionCtx, logCtx, flagsCtx, rangeStart, rangeEnd, personId) {
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, 'exams')) {
      const prevStatus = this.getApiStatus?.(sessionKey)?.exams;
    ...
    try {
      response = await webuntisApiService.getExams({
        ...
        normalizeTime: WebUntisClient.normalizeTimeToMinutes,
        sanitizeHtml: WebUntisClient.sanitizeHtmlText,
        debugApi: Boolean(debugApi),
        dumpRaw: Boolean(dumpRawApiResponses),
      });
    } catch (err) {
    ...
    if (sessionKey && response.status) {
      this._setApiStatus(sessionKey, 'exams', response.status);
    }
  ...
  async _getHomeworkViaRest(authCtx, sessionCtx, logCtx, flagsCtx, rangeStart, rangeEnd, personId) {
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, 'homework')) {
      const prevStatus = this.getApiStatus?.(sessionKey)?.homework;
    ...
    if (sessionKey && response.status) {
      this._setApiStatus(sessionKey, 'homework', response.status);
    }
  ...
  async _getAbsencesViaRest(authCtx, sessionCtx, logCtx, flagsCtx, rangeStart, rangeEnd, personId) {
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, 'absences')) {
      const prevStatus = this.getApiStatus?.(sessionKey)?.absences;
    ...
    if (sessionKey && response.status) {
      this._setApiStatus(sessionKey, 'absences', response.status);
    }
  ...
  async _getMessagesOfDayViaRest(authCtx, sessionCtx, logCtx, flagsCtx, date) {
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, 'messagesOfDay')) {
      const prevStatus = this.getApiStatus?.(sessionKey)?.messagesOfDay;
    ...
    if (sessionKey && response.status) {
      this._setApiStatus(sessionKey, 'messagesOfDay', response.status);
    }
  ...
  async fetchStudentData(params) {
    ...
    try {
      const apiStatus = this.getApiStatus(sessionKey);
