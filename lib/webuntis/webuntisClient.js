...
const { stripAllHtml, normalizeDateToInteger, normalizeTimeToHHMM, extractTimegridFromTimetable } = require('./dataOrchestration');
...
class WebUntisClient {
  ...
  _buildRestAuthHandlers(restCtx) {
    const { authService, qrCodeUrl, effectiveCacheKey, school, server, username, password, schoolYearId, authOptions, authRefreshTracker } =
      restCtx;
    ...
    try {
      ...
      if (httpStatus === 403) {
        mmLog(
          'debug',
          null,
          `[${endpoint}] HTTP 403 — endpoint not available (school may not have licensed this feature). Skipping for 24h.`
        );
      }
  ...
  _selectClassCandidate(candidates, preSelectedClassId, desiredName, isParentAccount) {
    ...
    if (!chosen && desiredName) {
      ...
      if (chosen) {
        this.mmLog('debug', null, `[REST] ✓ Matched className="${desiredName}" to classId=${chosen.id} (${chosen.name})`);
      } else {
