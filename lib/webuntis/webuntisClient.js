...
class WebUntisClient {
  ...
  static buildFetchFlags(wantsWidget, displayMode) {
    const wants = (name) => Boolean(typeof wantsWidget === 'function' && wantsWidget(name, displayMode));
  ...
  async fetchBundle(request) {
    ...
    const {
      ...
      fetchAbsences,
      fetchMessagesOfDay,
    } = WebUntisClient.buildFetchFlags(this.wantsWidget, effectiveDisplayMode);

    const fetchFlags = { fetchTimegrid, fetchTimetable, fetchExams, fetchHomeworks, fetchAbsences, fetchMessagesOfDay };
