...
const { maxAgeFor } = require("./responseCache");
...
} = require("./dataOrchestration");
const { orchestrateFetch } = require("./dataFetchOrchestrator");

/**
 * Core WebUntis client: resolves REST targets, runs the fetch orchestration and returns a
 * neutral bundle. Knows nothing about the MagicMirror payload contract.
 *
 * @param {Object} options
 * @param {Function} options.mmLog - (level, student, message) logger
 * @param {Function} [options.formatErr] - Error formatter
 * @param {Object} [options.apiStatus] - Endpoint status tracker with shouldSkip(sessionKey, endpoint),
 *   recordError(sessionKey, endpoint, err), recordStatus(sessionKey, endpoint, status) and
 *   getStatuses(sessionKey). Optional; without it every endpoint is always called.
 */
class WebUntisClient {
  constructor(options = {}) {
    this.mmLog = options.mmLog || (() => {});
    this.formatErr = options.formatErr || errorHandler.formatError;
    this.apiStatus = options.apiStatus || null;
    // Shared by every session of this helper (lib/webuntis/responseCache.js); null = no reuse.
    this.responseCache = options.responseCache || null;
    this.cacheManager =
  ...
  _shouldSkipApi(sessionKey, endpoint) {
    return Boolean(this.apiStatus?.shouldSkip?.(sessionKey, endpoint));
  }

  _recordApiStatusFromError(sessionKey, endpoint, err) {
    this.apiStatus?.recordError?.(sessionKey, endpoint, err);
  }

  _setApiStatus(sessionKey, endpoint, status) {
    this.apiStatus?.recordStatus?.(sessionKey, endpoint, status);
  }

  _getApiStatuses(sessionKey) {
    return this.apiStatus?.getStatuses?.(sessionKey) || {};
  }

  _extractRestRequestContext(authCtx, sessionCtx, logCtx, flagsCtx) {
    const { sessionKey, authRefreshTracker, responseMaxAgeMs } = sessionCtx || {};
    const { debugApi, dumpRawApiResponses } = flagsCtx || {};
    const { authService, qrCodeUrl, cacheKey, school, server, username, password, schoolYearId } = authCtx || {};
  ...
  _buildRestAuthHandlers(restCtx) {
    ...
    // schoolYearId is resolved once per fetch (see fetchBundle) against the date this fetch
    // actually targets; override whatever the shared auth cache carries (which reflects "today")
    // so a request for a different school year doesn't silently come back empty.
    const withResolvedSchoolYear = (authResultPromise) =>
      schoolYearId == null ? authResultPromise : authResultPromise.then((result) => ({ ...result, schoolYearId }));

    return {
      // Responses of this account may be reused by other sessions of it for responseMaxAgeMs.
      responseCache:
        this.responseCache && responseMaxAgeMs > 0
          ? { store: this.responseCache, scope: effectiveCacheKey, maxAgeMs: responseMaxAgeMs }
          : null,
      getAuth: () =>
        withResolvedSchoolYear(
          qrCodeUrl
  ...
  async _executeRestEndpoint(endpoint, restCtx, executeRequest) {
    ...
    if (sessionKey && this._shouldSkipApi(sessionKey, endpoint)) {
      const prevStatus = this._getApiStatuses(sessionKey)[endpoint];
      // The skip reason (permanent error vs. transient-failure backoff) is logged by the host's
      // shouldSkipApi implementation, which owns the status record.
      mmLog("debug", null, `[${endpoint}] Skipping API call due to previous status ${prevStatus}`);
      if (Number(prevStatus) === 403) {
        // Re-raise the licensing problem on every skipped cycle so the frontend keeps showing
        // the warning (it only displays non-critical warnings that persist across payloads).
        // `quiet` keeps wrapAsync from logging this as an error each cycle; the status record
        // is left untouched so the 24h retry window is not reset.
        const skipped = new Error(`[${endpoint}] endpoint unavailable (HTTP 403), skipped`);
        skipped.status = 403;
        skipped.code = "ENDPOINT_UNAVAILABLE";
        skipped.quiet = true;
        throw skipped;
      }
      return [];
    ...
    try {
      ...
      // Record the status first so the host's skip logic (24h for 403/404/410, circuit breaker
      // otherwise) applies on later cycles, then let the error propagate: the orchestrator's
      // wrapAsync() turns it into the empty default AND a user-visible warning. Swallowing the
      // 403 here used to leave the frontend with an empty collection and no explanation.
      if (sessionKey) this._recordApiStatusFromError(sessionKey, endpoint, err);

      const httpStatus = err?.status || err?.response?.status;
      if (httpStatus === 403) {
        mmLog("debug", null, `[${endpoint}] HTTP 403: endpoint not licensed for this school? Skipping for 24h.`);
      }
  ...
  async _fetchParentClassCandidates(authCtx, getAuth, rangeStart, rangeEnd, studentId, role) {
    ...
    this.mmLog(
      "debug",
      null,
      `[REST] Parent account (role=${role}): classes via classservices (studentId=${studentId})`,
    );

    try {
      const startDate = this._formatDateInt(rangeStart);
      const endDate = this._formatDateInt(rangeEnd);
      this.mmLog("debug", null, `[REST] classservices ${server} ${startDate}..${endDate} elementId=${studentId}`);
      ...
      if (map && typeof map === "object") {
        ...
        if (mappedClassId) {
          ...
          this.mmLog("debug", null, `[REST] personKlasseMap: studentId ${studentId} → classId ${preSelectedClassId}`);
        } else {
          this.mmLog("debug", null, `[REST] personKlasseMap: studentId ${studentId} not found in map`);
        }
      }
  ...
  async _fetchStandardClassCandidates(authCtx, getAuth, rangeStart, rangeEnd, role) {
    ...
    this.mmLog(
      "debug",
      null,
      `[REST] Student/teacher account (role=${role || "unknown"}): classes via timetable/filter`,
    );

    try {
      const start = this._formatDateISO(rangeStart);
      const end = this._formatDateISO(rangeEnd);
      this.mmLog("debug", null, `[REST] timetable/filter ${server} CLASS/STANDARD ${start}..${end}`);
  ...
  _selectClassCandidate(candidates, preSelectedClassId, desiredName, isParentAccount) {
    ...
    if (preSelectedClassId) {
      if (candidates.some((c) => Number(c.id) === Number(preSelectedClassId))) {
        ...
        this.mmLog("debug", null, `[REST] ✓ ${source} class: ${chosen.name} (classId=${preSelectedClassId})`);
      } else {
        this.mmLog("debug", null, `[REST] preSelected/mapped classId=${preSelectedClassId} not found in candidates`);
      }
    }
    ...
    if (!chosen && desiredName) {
      ...
      if (chosen) {
        this.mmLog(
          "debug",
          null,
          `[REST] ✓ Matched className="${desiredName}" to classId=${chosen.id} (${chosen.name})`,
        );
      } else {
        this.mmLog("debug", null, `[REST] className="${desiredName}" not found in candidates`);
      }
    }

    if (!chosen && !desiredName && candidates.length === 1) {
      chosen = candidates[0];
      this.mmLog("debug", null, `[REST] ✓ Auto-selected sole available class ${chosen.name} (classId=${chosen.id})`);
    }
    ...
    if (!chosen) {
      const available = candidates
        ...
        .filter(Boolean)
        .join(", ");
      const hint = desiredName
        ? `Class "${desiredName}" not found. Available: ${available}`
        : `Multiple classes available: ${available}`;
  ...
  async _resolveClassIdViaRest(authCtx, _sessionCtx, rangeStart, rangeEnd, className, role, options = {}) {
    ...
    if (isParentAccount && studentId) {
      ...
      ({ candidates, preSelectedClassId } = await this._fetchStandardClassCandidates(
        authCtx,
        getAuth,
        rangeStart,
        rangeEnd,
  ...
  async _getTimetableViaRest(authCtx, sessionCtx, logCtx, flagsCtx, rangeStart, rangeEnd, personId, options = {}) {
    ...
    return this._invokeRestEndpoint(
      "timetable",
      authCtx,
      sessionCtx,
      logCtx,
      flagsCtx,
      ({ server, authContext, debugApi, dumpRaw }) =>
    ...
    return this._invokeRestEndpoint(
      ...
      ({ server, authContext, debugApi, dumpRaw }) =>
        webuntisApiService.getExams({
          ...
          personId,
          logger: this.mmLog,
          normalizeDate: normalizeDateToInteger,
          normalizeTime: normalizeTimeToHHMM,
          stripHtml: stripAllHtml,
          debugApi,
          dumpRaw,
        }),
    );
  ...
  _createLogger(student, mmLog = this.mmLog) {
    return (...args) => {
      if (args.length >= 3 && typeof args[0] === "string" && typeof args[2] === "string") {
        const [level, studentCtx, message] = args;
        mmLog(level || "debug", studentCtx || student, message);
        return;
      }

      if (args.length === 2 && typeof args[0] === "string" && typeof args[1] === "string") {
        const [level, message] = args;
        mmLog(level || "debug", student, message);
        return;
      }

      const [msg] = args;
      mmLog("debug", student, typeof msg === "string" ? msg : JSON.stringify(msg));
    };
  }
  ...
  _logEmptyTargets(student, config, appData) {
    ...
    if (emptyStringCreds) {
      ...
      hint =
        "Student credentials are incomplete. Need username, password, school, and server together for direct student login.";
    ...
    } else if (hasParentCreds && hasStudentId && !appData?.user?.students) {
      hint =
    ...
    const message = `No REST targets built - cannot fetch data! ${hint}`;
    this.mmLog("warn", student, message);
    return message;
  }
  ...
  async fetchBundle(request) {
    ...
    // Logger of the requesting instance (its logLevel); the client's own serves shared paths.
    const mmLog = request.mmLog || this.mmLog;
    ...
    const logger = this._createLogger(student, mmLog);
    const logCtx = {
      logger,
      mmLog,
      formatErr: this.formatErr,
    ...
    // The school year cached at login always reflects "today"; re-resolve it against the date
    // this fetch actually targets (baseNow, i.e. debugDate when set) so requests for a different
    // school year - historical debugDate testing, or a real query placed near the year boundary -
    // are scoped correctly instead of silently coming back empty (see WebUntis's own
    // X-Webuntis-Api-School-Year-Id behavior).
    const resolvedSchoolYearId = await authService.getSchoolYearIdForDate(credKey, requestBaseNow, {
      token: authSession.token,
      cookieString: authSession.cookieString,
      server: authSession.server,
      fallbackSchoolYearId: authSession.schoolYearId,
    });
    ...
    const sessionCtx = {
      ...
      authRefreshTracker,
      authSession,
      responseMaxAgeMs: maxAgeFor(config),
    };

    const flagsCtx = requestFlagsCtx;

    const { school, server } = authSession;
    const restTargets = authService.buildRestTargets({
      student,
      moduleConfig: config,
      school,
      server,
      ownPersonId: authSession.personId,
    ...
    if (restTargets && restTargets.length > 0) {
      ...
      mmLog("debug", student, `REST targets: ${targetSummaries.join(", ")}`);
    }

    // An empty target list is almost always a configuration problem (typically a missing
    // studentId on a parent login). The fetch below then yields status 'unavailable' for every
    // data type, which says *that* nothing arrived but not *why* - so carry the diagnosis into
    // the payload as a config warning instead of leaving it in the log only.
    const configWarnings = [];
    if (!restTargets || restTargets.length === 0) {
      const emptyTargetsWarning = this._logEmptyTargets(student, config, authSession.appData);
      if (emptyTargetsWarning) {
        configWarnings.push(emptyTargetsWarning);
      }
    }
    ...
    if (fetchTimegrid && grid.length === 0 && timetable.length > 0) {
      grid = extractTimegridFromTimetable(timetable);
    }
    ...
    const findHolidayForDate = (ymd, holidays) => {
      ...
      return (
        holidays.find((holiday) => Number(holiday.startDate) <= dateNum && dateNum <= Number(holiday.endDate)) || null
      );
    };
    ...
    mmLog(
      "debug",
      student,
      `Fetched: ${timetable.length} lesson(s), ${rawExams.length} exam(s), ${hwResult.length} homework(s), ${rawAbsences.length} absence(s), ${rawMessagesOfDay.length} message(s)`,
    );
    ...
    return {
      ...
      configWarnings,
      fetchFlags,
      apiStatus: this._getApiStatuses(sessionKey),
      apiRecords: this.apiStatus?.getRecords?.(sessionKey) || {},
      data: {
        grid,
        timetable,
        rawExams,
