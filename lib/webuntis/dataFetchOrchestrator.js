...
async function orchestrateFetch(params) {
  const { student, dateRanges, baseNow, restTargets, contexts = {}, restFns = {}, fetchFlags, logger } = params;
  ...
  if (fetchMessagesOfDay) {
    ...
    fetchPromises.push(
      (async () => {
        for (const target of restTargets) {
          const targetAuthCtx = buildTargetAuthCtx(target);
          const result = await wrapAsync(() => callWithContext(getMessagesOfDayViaRest, targetAuthCtx, baseNow), {
            logger: activeLogger,
