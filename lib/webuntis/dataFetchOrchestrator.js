...
async function fetchTimetableFirst({
  fetchTimetable,
  dateRanges,
  activeLogger,
  fetchFromTargets,
...
async function runAuthRefreshRetry({ tracker, alreadyRetried, activeLogger, params }) {
  ...
  activeLogger?.(
    "warn",
    null,
    "[fetch] Auth refresh detected during timetable fetch, retrying all data types with fresh token",
  );
...
function buildTargetHelpers(ctx) {
  ...
  const callWithContext = (fn, targetAuthCtx, ...args) =>
    callRest(fn, targetAuthCtx, sessionCtx, logCtx, flagsCtx, ...args);
  ...
  const fetchFromTargets = async ({
    ...
    for (const target of targets) {
      ...
      const mapped = await wrapAsync(
        async () => (typeof mapResult === "function" ? await mapResult(result, target) : result),
        {
          logger: activeLogger,
          context: {
            dataType,
...
async function runAuthCanaryIfNeeded(ctx, helpers) {
  const { student, dateRanges, baseNow, restTargets, fetchFlags, activeLogger, contexts } = ctx;
  ...
  const authService = contexts?.authCtx?.authService;
  const cacheKey = contexts?.authCtx?.cacheKey;
  if (authService?.wasTimetableRecentlyVerified?.(cacheKey)) {
    activeLogger?.("debug", null, "[fetch] Skipping auth canary; a sibling session already verified this account");
    return;
  }
  ...
  await helpers.callTargetWithWrap({
    ...
    execute: (targetAuthCtx) =>
      helpers
        ...
        .then((result) => {
          authService?.markTimetableVerified?.(cacheKey);
          return result;
        }),
  });
}
...
function buildParallelFetchPlans(ctx, helpers) {
  const { dateRanges, baseNow, fetchFlags, hwNextDays, hwPastDays, activeLogger, restTargets } = ctx;
  ...
  const studentScopedTargets = Array.isArray(restTargets)
    ? restTargets.filter((target) => roleToResourceType(target?.role) !== "TEACHER")
    : [];
  const hasTeacherOnlyTargets =
    Array.isArray(restTargets) && restTargets.length > 0 && studentScopedTargets.length === 0;

  const fetchPlans = [];

  if (fetchExams && dateRanges.exams.nextDays > 0) {
    if (hasTeacherOnlyTargets) {
      activeLogger?.("debug", null, "Exams: skipped for teacher target(s); current endpoint is student-scoped");
    } else {
      fetchPlans.push({
        type: "exams",
        promise: helpers.fetchFromTargets({
          dataType: "exams",
          defaultValue: [],
          targets: studentScopedTargets.length > 0 ? studentScopedTargets : restTargets,
          execute: (targetAuthCtx, target) =>
            helpers.callWithContext(
              getExamsViaRest,
              targetAuthCtx,
  ...
  if (fetchHomeworks) {
    if (hasTeacherOnlyTargets) {
      activeLogger?.("debug", null, "Homework: skipped for teacher target(s); current endpoint is student-scoped");
    } else {
      fetchPlans.push({
        type: "homework",
        promise: helpers.fetchFromTargets({
  ...
  if (fetchAbsences) {
    if (hasTeacherOnlyTargets) {
      activeLogger?.("debug", null, "Absences: skipped for teacher target(s); current endpoint is student-scoped");
    } else {
      fetchPlans.push({
        type: "absences",
        promise: helpers.fetchFromTargets({
