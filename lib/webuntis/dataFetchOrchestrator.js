...
function buildTargetHelpers(ctx) {
  ...
  const fetchFromTargets = async ({ dataType, defaultValue, execute, isAcceptable, mapResult, targets = restTargets }) => {
...
async function fetchTimetablePhase(ctx, helpers) {
  ...
  const buildTimetableRequest = (targetAuthCtx, target) => {
    const resolvedResourceType = roleToResourceType(target.role);
    return helpers
      .callWithContext(getTimetableViaRest, targetAuthCtx, dateRanges.timetable.start, dateRanges.timetable.end, target.personId, {
        useClassTimetable: Boolean(student.useClassTimetable),
        className: ctx.className,
        classId: student.classId || null,
        personId: target.personId,
...
function buildParallelFetchPlans(ctx, helpers) {
  ...
  const studentScopedTargets = Array.isArray(restTargets)
    ...
    : [];
  const hasTeacherOnlyTargets = Array.isArray(restTargets) && restTargets.length > 0 && studentScopedTargets.length === 0;
