...
function convertRestErrorToWarning(error, context = {}) {
  ...
  if (code === 'ETIMEDOUT' || code === 'ERR_HTTP_REQUEST_TIMEOUT') {
    return `WebUntis connection timeout for "${studentTitle}". Server is slow or overloaded. Will retry automatically.`;
  }

  // Connection reset by peer — temporary network issue
  if (code === 'ECONNRESET') {
    return `WebUntis connection reset for "${studentTitle}". Will retry automatically.`;
  }
  ...
  if (msg.includes('timeout')) {
    return `WebUntis connection timeout for "${studentTitle}". Will retry automatically on next fetch.`;
  }
  if (
    ...
    return `Network error connecting to WebUntis server "${server}". Will retry automatically on next fetch.`;
  }

  // Service unavailable (5xx errors) — temporary server issues
  if (status === 500 || status === 502 || status === 503 || status === 504) {
    return `WebUntis server error (HTTP ${status}) for "${studentTitle}". This is temporary. Retrying automatically...`;
  }
