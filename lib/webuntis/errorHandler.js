...
const AUTH_ERROR_CODES = new Set(["AUTH_FAILED", "SESSION_EXPIRED", "TOKEN_REQUEST_FAILED", "TOKEN_INVALID"]);

/**
 * Whether an error means "the WebUntis session/credentials are not usable".
 *
 * Covers tagged auth errors from httpClient/fetchClient (`isAuthError`, auth codes) as well as a
 * plain HTTP 401. Login failures reach us with `httpStatus: 200` (JSON-RPC reports them inside a
 * 200 body), so callers must check this BEFORE looking at the numeric status.
 *
 * @param {Error|Object} error - Error object
 * @returns {boolean} True for authentication/session errors
 */
function isAuthError(error) {
  ...
  const statusRaw =
    error?.response?.status ?? error?.status ?? error?.httpStatus ?? error?.cause?.status ?? error?.cause?.httpStatus;
  return Number(statusRaw) === 401;
}
...
function convertRestErrorToWarning(error, context = {}) {
  ...
  // Session expired / login redirect / HTML instead of data: the session is gone and the
  // automatic re-login did not (yet) succeed.
  if (code === "SESSION_EXPIRED") {
    return `WebUntis session expired${dataTypeContext} for "${studentTitle}". Re-login is attempted on the next fetch.`;
  }

  // Authentication errors: HTTP 401, or a login attempt rejected by WebUntis. Login rejections
  // arrive with httpStatus 200 (JSON-RPC error body), so this must not depend on `status`.
  if (isAuthError(error)) {
    return `Authentication failed${dataTypeContext} for "${studentTitle}": ${error.message || "Invalid credentials or insufficient permissions."}`;
  }

  // Structurally unusable response (e.g. timetable body without `days`)
  if (code === "INVALID_RESPONSE") {
    return `WebUntis returned an unusable response${dataTypeContext} for "${studentTitle}": ${error.message || "unexpected structure"}. Retrying on next fetch.`;
  }
