...
function loadPluginRegistry(options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (missingPath) {
      warnings.push(
        `[plugins] Plugin "${validation.manifest.id}" is missing entry file "${pathImpl.relative(pluginRoot, missingPath)}".`,
      );
      continue;
    }
  ...
  const filteredPlugins = plugins.filter((plugin) => {
    const aliases = Array.isArray(plugin.manifest?.activation?.displayAliases)
      ? plugin.manifest.activation.displayAliases
      : [];
