...
function isConfiguredStudentCandidate(student) {
  ...
  const hasStudentId =
    student.studentId !== undefined && student.studentId !== null && String(student.studentId).trim() !== "";
  ...
  if ((!configStudent.studentId || configStudent.studentId === "") && configStudent.title) {
    ...
    if (candidateIds.length === 1) {
      ...
      logger(
        "debug",
        configStudent,
        `Auto-assigned studentId=${candidateIds[0]} for "${configStudent.title}" (only match found)`,
      );
    } else {
...
async function ensureStudentsFromAppData(moduleConfig, { authService, logger, formatError }) {
  try {
    ...
    const configuredStudents = (Array.isArray(moduleConfig.students) ? moduleConfig.students : []).filter(
      isConfiguredStudentCandidate,
    );
    const hasParentCreds = Boolean(
      (moduleConfig.username && moduleConfig.password && moduleConfig.school) || moduleConfig.qrcode,
    );
      ...
      if (hasParentCreds) {
        try {
          ...
          logger(
            "warn",
            null,
            `Auto-discovered names for title fallback unavailable (server=${server}, reachable?): ${formatError(err)}`,
          );
        }
      }
