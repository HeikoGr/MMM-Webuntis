...
function applyLegacyMappings(config) {
  ...
  LEGACY_MAPPINGS.forEach((mapping) => {
    ...
    // Top-level config mappings
    if (mapping.context === 'config' && out[mapping.new] === undefined) {
    ...
    if (mapping.context === 'widget' && mapping.widgets) {
      mapping.widgets.forEach((widget) => {
        if (out[mapping.old] !== undefined && out[widget][mapping.new] === undefined) {
          const transformed = mapping.transform ? mapping.transform(out[mapping.old]) : out[mapping.old];
...
function generateDeprecationWarnings(legacyKeys) {
  ...
  // Map of legacy keys to detailed migration instructions
  const deprecationInfo = {
    // Top-level legacy keys
    days: {
      new: 'grid.nextDays / lessons.nextDays',
      message: 'The "days" config is deprecated. Use widget-specific "nextDays" instead (e.g., grid.nextDays, lessons.nextDays).',
    },
    daysToShow: {
      new: 'grid.nextDays / lessons.nextDays',
      message: 'The "daysToShow" config is deprecated. Use widget-specific "nextDays" instead.',
    },
    pastDays: {
      new: 'grid.pastDays / homework.pastDays / absences.pastDays',
      message: 'The top-level "pastDays" is deprecated. Use widget-specific pastDays (e.g., grid.pastDays).',
    },
    examsDays: {
      new: 'exams.nextDays',
      message: 'The "examsDays" config is deprecated. Use "exams.nextDays" instead.',
    },
    mergeGapMin: {
      new: 'grid.mergeGap',
      message: 'The "mergeGapMin" config is deprecated. Use "grid.mergeGap" instead.',
    },
    mergeGapMinutes: {
      new: 'grid.mergeGap',
      message: 'The "mergeGapMinutes" config is deprecated. Use "grid.mergeGap" instead.',
    },
    displaymode: {
      new: 'displayMode',
      message: 'The "displaymode" config (lowercase) is deprecated. Use "displayMode" (camelCase) instead.',
    },
    debug: {
      new: 'logLevel: "debug"',
      message: 'The "debug" config is deprecated. Use "logLevel: \'debug\'" instead.',
    },
    enableDebug: {
      new: 'logLevel: "debug"',
      message: 'The "enableDebug" config is deprecated. Use "logLevel: \'debug\'" instead.',
    },
    fetchInterval: {
      new: 'updateInterval',
      message: 'The "fetchInterval" config is deprecated. Use "updateInterval" instead.',
    },
    showAbsences: {
      new: 'displayMode',
      message:
        'The "showAbsences" config is deprecated. Use "displayMode" to control which widgets are shown (e.g., "grid, lessons, exams, homework, absences").',
    },
    showHomework: {
      new: 'displayMode',
      message:
        'The "showHomework" config is deprecated. Use "displayMode" to control which widgets are shown (e.g., "grid, lessons, exams, homework").',
    },
    showExams: {
      new: 'displayMode',
      message: 'The "showExams" config is deprecated. Use "displayMode" to control which widgets are shown (e.g., "grid, lessons, exams").',
    },
    // Widget-specific deprecated keys
    daysAhead: {
      new: 'exams.nextDays',
      message: 'The "exams.daysAhead" config is deprecated. Use "exams.nextDays" instead for consistency.',
    },
    maxGridLessons: {
      new: 'grid.maxLessons',
      message: 'The "maxGridLessons" config is deprecated. Use "grid.maxLessons" instead.',
    },
  };

  for (const key of legacyKeys) {
    const info = deprecationInfo[key];
    if (info) {
      warnings.push(`⚠️ DEPRECATED: "${key}" → Use "${info.new}" instead. ${info.message}`);
    } else {
      ...
      if (mapping && mapping.new) {
        ...
        warnings.push(`⚠️ DEPRECATED: "${key}" has been renamed to "${target}". Please update your config.`);
      } else {
        warnings.push(`⚠️ DEPRECATED: "${key}" is no longer supported. Please remove it from your config.`);
      }
    }
  }
