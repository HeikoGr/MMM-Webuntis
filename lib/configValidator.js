...
function applyLegacyMappings(config) {
  ...
  // Handle global dateFormat mapping
  const handleGlobalDateFormat = () => {
    const globalDateFormat = out.dateFormat;

    // Apply global dateFormat to all widgets if not already set
    if (globalDateFormat) {
      const widgets = ['lessons', 'grid', 'exams', 'homework', 'absences', 'messagesofday'];
      widgets.forEach((widget) => {
        if (!out[widget].dateFormat) {
          out[widget].dateFormat = globalDateFormat;
        }
      });
