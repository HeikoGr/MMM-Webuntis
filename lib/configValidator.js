  ...
  students: {
    ...
    items: {
      ...
      properties: {
        ...
        school: { type: 'string', required: true, description: 'School identifier (server)' },
        username: { type: 'string', required: true, description: 'WebUntis username' },
        password: { type: 'string', required: true, description: 'WebUntis password' },
        studentId: {
          type: 'number',
...
function validateStudent(student, index) {
  const errors = [];
  const requiredFields = ['title', 'school', 'username', 'password'];
