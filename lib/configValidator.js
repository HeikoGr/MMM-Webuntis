 ...
 * @param {Object} options - Optional { warnCallback } for deprecation warnings
 * @returns {Object} { normalizedConfig, legacyUsed }
 */
function applyLegacyMappings(config, options = {}) {
  if (!config || typeof config !== 'object') return { normalizedConfig: config, legacyUsed: [] };

  const out = { ...config };
  const legacyUsed = [];
  const { warnCallback } = options;
  ...
  // Emit deprecation warnings if callback provided
  if (legacyUsed.length > 0 && warnCallback) {
    const uniq = Array.from(new Set(legacyUsed));
    warnCallback(`⚠️ Deprecated config keys detected: ${uniq.join(', ')}. Please update to new config format.`);
  }
