 ...
 */
async function callWebUntisAPI({ dataType, getAuth, onAuthError, server, params, logger, transform, debugApi = false }) {
  ...
  const makeRequest = async () => {
    ...
    return await restClient.callRestAPI({
      server,
      path: endpoint,
      method: 'GET',
      params,
  ...
  try {
    ...
    if (isAuthError(error) && onAuthError) {
      if (logger) logger('debug', null, `${logPrefix} Auth error detected, invalidating cache and retrying...`);
      ...
      try {
        ...
        const data = await makeRequest();
        const result = transform ? transform(data) : data;
        if (logger) logger('debug', null, `${logPrefix} Retry successful: ${Array.isArray(result) ? result.length : '?'} items`);
        return result;
      } catch (retryError) {
        if (logger) logger('error', null, `${logPrefix} Retry failed: ${retryError.message}`);
        throw retryError;
      }
    }
 ...
 */
async function getHomework({ getAuth, onAuthError, server, rangeStart, rangeEnd, studentId, logger, debugApi = false }) {
  const params = {
    startDate: formatDateYYYYMMDD(rangeStart),
    endDate: formatDateYYYYMMDD(rangeEnd),
  };
 ...
 */
async function getAbsences({ getAuth, onAuthError, server, rangeStart, rangeEnd, studentId, logger, debugApi = false }) {
  const params = {
    startDate: formatDateYYYYMMDD(rangeStart),
    endDate: formatDateYYYYMMDD(rangeEnd),
    studentId: studentId ?? -1,
 ...
 */
async function getMessagesOfDay({ getAuth, onAuthError, server, date, logger, debugApi = false }) {
