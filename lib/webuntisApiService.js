 ...
 */
async function callWebUntisAPI({ dataType, getAuth, server, params, logger, transform }) {
 ...
 */
async function getExams({ getAuth, server, rangeStart, rangeEnd, studentId, logger, normalizeDate, normalizeTime, sanitizeHtml }) {
  const params = {
 ...
 */
async function getHomework({ getAuth, server, rangeStart, rangeEnd, studentId, logger }) {
  const params = {
    startDate: formatDateYYYYMMDD(rangeStart),
    endDate: formatDateYYYYMMDD(rangeEnd),
  };
 ...
 */
async function getAbsences({ getAuth, server, rangeStart, rangeEnd, studentId, logger }) {
  const params = {
    startDate: formatDateYYYYMMDD(rangeStart),
    endDate: formatDateYYYYMMDD(rangeEnd),
    studentId: studentId ?? -1,
  ...
  const transform = (data) => {
    ...
    absArr.forEach((abs) => {
      absences.push({
        ...
        su: abs.su?.[0] ? [{ name: abs.su[0].name, longname: abs.su[0].longname }] : [],
        te: abs.te?.[0] ? [{ name: abs.te[0].name, longname: abs.te[0].longname }] : [],
 ...
 */
async function getMessagesOfDay({ getAuth, server, date, logger }) {
  // Note: messagesofday uses direct axios call in callWebUntisAPI, no tenantId/schoolYearId needed
  const params = {
