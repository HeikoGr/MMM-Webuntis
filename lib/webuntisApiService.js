 ...
 * @param {Object} config.authService - Optional authService instance for session-wide auth coordination
 * @param {string} config.sessionKey - Optional session key for coordinating auth across requests
...
async function callWebUntisAPI({
  ...
  // PROACTIVE TOKEN CHECK: Wait for any session-wide auth to complete before making API call
  // This prevents API calls with expiring/refreshing tokens
  if (authService && sessionKey && typeof authService.waitForSessionAuth === 'function') {
...
async function getTimetable({
  ...
  // Auto-detect resourceType if not provided (backwards compatibility)
  const effectiveResourceType = resourceType || (useClassTimetable ? 'CLASS' : 'STUDENT');
  ...
  function mapPositionsToFields(entry) {
    ...
    const result = { te: [], su: [], ro: [], cl: [], sg: [], info: [] };
        ...
        if (day.gridEntries && Array.isArray(day.gridEntries)) {
          day.gridEntries.forEach((entry) => {
            ...
            const lesson = {
              ...
              te: mappedFields.te, // Teachers
              su: mappedFields.su, // Subjects
              ro: mappedFields.ro, // Rooms
              ...
              type: entry.type || null, // EXAM, NORMAL_TEACHING_PERIOD, etc. (kept separate from activityType)
              ...
              icons: Array.isArray(entry.icons) ? entry.icons : [], // Icons from API (e.g., ["HOMEWORK", "NOTES"])
            };
            lessons.push(lesson);
          });
        }
 ...
 */
async function getMessagesOfDay({
  getAuth,
  onAuthError,
  server,
  date,
