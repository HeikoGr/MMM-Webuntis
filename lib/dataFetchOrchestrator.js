...
async function orchestrateFetch(params) {
  ...
  const { wrapAsync } = require('./webuntis/errorUtils');
  ...
  // ===== STEP 0: Auth canary when timetable is not requested =====
  // Some endpoints return 200 OK with empty arrays on expired tokens.
  // If timetable is disabled, run a minimal timetable call as a token canary.
  ...
  if (fetchTimetable && dateRanges.timetable.nextDays > 0) {
    activeLogger?.(`🔍 Fetching timetable first (token validation)...`);
    for (const target of restTargets) {
      const resolvedResourceType = roleToResourceType(target.role);
      const targetAuthCtx = {
        ...authCtx,
      ...
      if (activeLogger) {
        activeLogger(
          'debug',
          null,
          `[timetable] Role check: ownRole=${sessionCtx?.authSession?.role || 'unknown'}, targetRole=${target.role || 'unknown'}, resourceType=${resolvedResourceType || 'default'}, personId=${target.personId || 'n/a'}`
        );
      }
  ...
  if (fetchExams && dateRanges.exams.nextDays > 0) {
    ...
    activeLogger?.(`Exams: skipped (exams.nextDays=${dateRanges.exams.nextDays})`);
  }
  ...
  if (fetchPromises.length > 0) {
    activeLogger?.(`⚡ Fetching ${fetchPromises.length} remaining data types in parallel...`);
