...
async function orchestrateFetch(params) {
  ...
  // Starting parallel fetch silently (logs removed for cleaner output)
  ...
  // If any request triggered an auth refresh, rerun once with fresh auth so all data types use the new token
  ...
  if (tracker?.refreshed && !alreadyRetried) {
    tracker.refreshed = false; // reset before retry to detect persistent auth churn
    if (logger) logger('warn', null, '[fetch] Auth refresh detected during parallel fetch, retrying all data types with fresh token');
