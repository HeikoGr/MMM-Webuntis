    ...
    for (const target of restTargets) {
      ...
      if (logger) {
        logger(
          'debug',
          null,
          `[timetable] Role check: ownRole=${restOptions?.authSession?.role || 'unknown'}, targetRole=${target.role || 'unknown'}, resourceType=${resolvedResourceType || 'default'}, personId=${target.personId || 'n/a'}`
        );
      }
