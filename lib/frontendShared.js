 ...
 */
(() => {
  const root = window.MMMWebuntisFrontendShared || {};
  window.MMMWebuntisFrontendShared = root;
  ...
  // Single source of truth for the frontend default timezone. Previously hardcoded in every
  // plugin's own getCurrentDateContext() copy.
  const DEFAULT_TIMEZONE = "Europe/Berlin";

  let dateContextFallbackWarned = false;

  /**
   * Global log function for widgets
   * Writes through MagicMirror's Log (global logLevel); window.MMMWebuntisLogLevel, set by the
   * main module, can only narrow it. Unset = the global level alone.
   *
  ...
  function log(level, ...args) {
    ...
    const configured = LOG_LEVEL_WEIGHTS[window.MMMWebuntisLogLevel];
    const msgLevel = LOG_LEVEL_WEIGHTS[level] !== undefined ? level : "info";
    if (configured !== undefined && LOG_LEVEL_WEIGHTS[msgLevel] > configured) return;

    const sink = window.Log || console;
    (sink[msgLevel] || sink.log).call(sink, `[MMM-Webuntis] [${msgLevel.toUpperCase()}]`, ...args);
  }
  ...
  /**
   * Resolve the current date context, honouring a configured `debugDate`.
   *
   * Delegates to the canonical implementation in `lib/runtime-utils.js`. If that global is
   * unavailable, falls back to the real wall-clock date.
   *
   * The fallback cannot honour `debugDate`, so it warns once instead of silently rendering
   * today's date - a silent fallback here is very hard to diagnose, because every date-driven
   * plugin simply shows "today" while the config says otherwise.
   *
   * @param {Object} config - Config source that may carry `debugDate`
   * @param {Object} [options] - Overrides, e.g. `{ defaultTimezone }`
  ...
  function getCurrentDateContext(config, options = {}) {
    const timezone = options.defaultTimezone || DEFAULT_TIMEZONE;
    const runtimeUtils = window.MMModuleRuntimeUtils;

    if (runtimeUtils && typeof runtimeUtils.getCurrentDateContext === "function") {
      return runtimeUtils.getCurrentDateContext(config || {}, { defaultTimezone: timezone });
    }

    if (!dateContextFallbackWarned) {
      dateContextFallbackWarned = true;
      log("warn", "[shared] MMModuleRuntimeUtils unavailable - falling back to wall clock; debugDate is ignored");
    }
  ...
  /**
   * Expand holiday ranges into a lookup keyed by YYYYMMDD.
   *
   * @param {Array} holidays - Holiday ranges with `startDate`/`endDate` as YYYYMMDD numbers
   * @returns {Object} Map of YYYYMMDD to the covering holiday
   */
  ...
  /**
   * Index day notices by their YYYYMMDD date.
   *
   * @param {Array} dayNotices - Day notice entries carrying a numeric `date`
   * @returns {Object} Map of YYYYMMDD to notice
   */
   ...
   *
   * @param {Object} widgetRoot - The MMMWebuntisFrontendShared root object
   * @returns {Object} Object containing util and dom helper functions
   */
  /**
   * Find the timetable period a lesson starting at `startMin` belongs to.
   *
   * A lesson inside a period belongs to it. One starting in a break (or before the first period)
   * belongs to the next period - it cannot have taken place in an earlier one - and one starting
   * after the last period to the last one.
   *
   * @param {number} startMin - Lesson start in minutes since midnight
   * @param {Array<{startMin: number, endMin?: number}>} timeUnits - Periods in chronological order
   * @returns {number} Period index, or -1 without periods
   */
  function findPeriodIndex(startMin, timeUnits) {
    if (!Array.isArray(timeUnits) || timeUnits.length === 0) return -1;

    const containing = timeUnits.findIndex((unit, index) => {
      const end = unit.endMin ?? timeUnits[index + 1]?.startMin ?? unit.startMin + 60;
      return startMin >= unit.startMin && startMin < end;
    });
    if (containing !== -1) return containing;

    const next = timeUnits.findIndex((unit) => unit.startMin > startMin);
    return next === -1 ? timeUnits.length - 1 : next;
  }
  ...
  function resolveWidgetHelpers(widgetRoot) {
    ...
    return {
      ...
      findPeriodIndex: requireFunction("util.findPeriodIndex", util.findPeriodIndex),
      ...
      initializeWidgetContextAndHeader: requireFunction(
        "util.initializeWidgetContextAndHeader",
        util.initializeWidgetContextAndHeader,
      ),
  ...
  function widgetDisplayName(ctx, widgetName) {
    const t = (key, fallback) => ctx?.translate?.(key, undefined, fallback) || fallback;
  ...
  // The fallback is only used when the key has no translation at all - a translation that
  // happens to equal its key ("weekend": "weekend") is still a translation.
  function translateDayState(ctx, key, fallback) {
    return ctx?.translate?.(key, undefined, fallback) || fallback;
  }
  ...
  function getEmptyDayState(ctx, studentTitle, dayValue) {
    ...
    if (holiday) {
      return {
        ...
        inlineIconClass: "wu-inline-icon wu-inline-icon--lesson wu-inline-icon--holiday",
      };
    }
    ...
    if (noticeKind === "timetable-restricted" || noticeStatus === "NOT_ALLOWED") {
      return {
        ...
        dateYmd,
        label: translateDayState(ctx, "timetable-restricted", "locked"),
        noticeType: "timetable-restricted",
        rowClass: "empty-day-notice",
        inlineIconClass: "wu-inline-icon wu-inline-icon--warning",
      };
    }
    ...
    if ([0, 6].includes(dayDate.getDay())) {
      return {
        type: "weekend",
        dateYmd,
        label: translateDayState(ctx, "weekend", "weekend"),
    ...
    // No lessons for this day and the timetable collection could not be fetched (and nothing
    // older is being shown): the day is unknown, not lesson-free.
    const lessonsState = ctx?.collectionStateByStudent?.[studentTitle]?.lessons || null;
    if (lessonsState && lessonsState.status === "unavailable" && !lessonsState.stale) {
      return {
        type: "unavailable",
        dateYmd,
        label: translateDayState(ctx, "unavailable", "data unavailable"),
        noticeType: "unavailable",
        rowClass: "empty-day-notice unavailable-notice",
        inlineIconClass: "wu-inline-icon wu-inline-icon--warning",
      };
    }
    ...
    return {
      type: "no-lessons",
      dateYmd,
      // WebUntis marks lesson-free days as NO_DATA; without that notice the day merely has no entries.
      confirmed: noticeKind === "no-data" || noticeStatus === "NO_DATA",
      label: translateDayState(ctx, "no-lessons", "no lessons"),
      noticeType: "no-lessons",
      rowClass: "empty-day-notice",
      inlineIconClass: "wu-inline-icon wu-inline-icon--no-lessons",
    };
  ...
  function isPopoverValueVisible(value) {
  ...
  function stringifyPopoverValue(value) {
    if (!isPopoverValueVisible(value)) return "";
    if (typeof value === "string") return value.trim();
    if (typeof value === "number" || typeof value === "boolean") return String(value);
    if (Array.isArray(value)) {
      return value
        .map((item) => stringifyPopoverValue(item))
        .filter(Boolean)
        .join(", ");
    }
  ...
  function createPopoverController(options = {}) {
    const mountRoot = options?.mountRoot?.appendChild ? options.mountRoot : document.body;
    const baseClassName = String(options?.baseClassName || "wu-shared-popover").trim();
    const presentation =
      String(options?.presentation || "modal")
        .trim()
        .toLowerCase() === "anchored"
        ? "anchored"
        : "modal";
    ...
    let currentToggleKey = null;

    const popover = document.createElement("div");
    popover.className = baseClassName;
    popover.classList.add(presentation === "modal" ? "is-modal" : "is-anchored");
    popover.setAttribute("role", "dialog");
    popover.setAttribute("aria-modal", presentation === "modal" ? "true" : "false");
    popover.setAttribute("aria-hidden", "true");
    popover.style.display = "none";

    const shell = document.createElement("div");
    shell.className = "wu-shared-popover__shell";

    const header = document.createElement("header");
    header.className = "wu-shared-popover__header";

    const headingWrap = document.createElement("div");
    headingWrap.className = "wu-shared-popover__heading-wrap";

    const titleEl = document.createElement("h3");
    titleEl.className = "wu-shared-popover__title";

    const subtitleEl = document.createElement("div");
    subtitleEl.className = "wu-shared-popover__subtitle";
    ...
    closeBtn.className = "wu-shared-popover__close";
    ...
    body.className = "wu-shared-popover__body";

    shell.appendChild(header);
    shell.appendChild(body);
    popover.appendChild(shell);
    mountRoot.appendChild(popover);

    function setViewportListeners(enable) {
      if (enable === viewportListenersActive) return;
      viewportListenersActive = enable;

      const method = enable ? "addEventListener" : "removeEventListener";
      if (presentation === "anchored") {
        window[method]("resize", reposition, { passive: true });
        window[method]("scroll", reposition, { passive: true, capture: true });
      }
      document[method]("keydown", onKeyDown, true);
      document[method]("mousedown", onDocumentPointerDown, true);
      document[method]("touchstart", onDocumentPointerDown, { passive: true, capture: true });
    }
    ...
    function reposition() {
      ...
      if (!isOpen || !anchorEl?.isConnected) {
        close();
        return;
      }
      ...
      const fallbackTop = anchorRect.top - popRect.height - 8;
      const top =
        preferredTop + popRect.height <= window.innerHeight - margin
          ? preferredTop
          : Math.max(margin, Math.min(fallbackTop, window.innerHeight - popRect.height - margin));
    ...
    function buildSectionNode(section) {
      ...
      sectionNode.className = "wu-shared-popover__section";

      const title = String(section?.title || "").trim();
      if (title) {
        const heading = document.createElement("h4");
        heading.className = "wu-shared-popover__section-title";
      ...
      dl.className = "wu-shared-popover__list";

      entries.forEach((entry) => {
        const value = stringifyPopoverValue(entry?.value);
        if (!value) return;

        const label = String(entry?.label || "").trim();
        if (label) {
          const dt = document.createElement("dt");
          dt.className = "wu-shared-popover__term";
          dt.textContent = label;
          dl.appendChild(dt);
        }

        const dd = document.createElement("dd");
        dd.className = "wu-shared-popover__desc";
        dd.textContent = value;
        dl.appendChild(dd);
      });

      if (dl.childElementCount > 0) {
        sectionNode.appendChild(dl);
      }
    ...
    function open(payload = {}) {
      anchorEl = presentation === "anchored" && payload?.anchorEl?.getBoundingClientRect ? payload.anchorEl : null;
      currentToggleKey = payload?.toggleKey ?? null;

      const title = String(payload?.title || "").trim();
      const subtitle = String(payload?.subtitle || "").trim();
      const closeLabel = String(payload?.closeLabel || "Close details").trim();
      const popoverClassName = String(payload?.className || "").trim();
      const sections = Array.isArray(payload?.sections) ? payload.sections : [];

      popover.className = baseClassName;
      popover.classList.add(presentation === "modal" ? "is-modal" : "is-anchored");
      if (popoverClassName) popover.classList.add(popoverClassName);

      titleEl.textContent = title || "Details";
      subtitleEl.textContent = subtitle;
      subtitleEl.style.display = subtitle ? "" : "none";
      closeBtn.setAttribute("aria-label", closeLabel || "Close details");

      body.replaceChildren();

      sections.forEach((section) => {
        const sectionNode = buildSectionNode(section);
        if (sectionNode.childElementCount > 0) {
          body.appendChild(sectionNode);
        }
      });

      const customContent = payload?.contentNode;
      ...
      if (rawData !== undefined) {
        ...
        details.className = "wu-shared-popover__raw";
        const summary = document.createElement("summary");
        summary.textContent = String(payload?.rawLabel || "Raw data");
        const pre = document.createElement("pre");
        pre.className = "wu-shared-popover__raw-pre";
        try {
          pre.textContent = JSON.stringify(rawData, null, 2);
        } catch {
          pre.textContent = String(rawData);
        }
      ...
      if (body.childElementCount === 0) {
        ...
        empty.className = "wu-shared-popover__empty";
        empty.textContent = String(payload?.emptyLabel || "No details available.");
    ...
    function toggle(payload = {}) {
      const nextToggleKey = payload?.toggleKey ?? null;
      if (isOpen && nextToggleKey !== null && currentToggleKey !== null && nextToggleKey === currentToggleKey) {
        close();
        return;
      }
      if (isOpen && presentation === "anchored" && payload?.anchorEl && anchorEl === payload.anchorEl) {
        close();
        return;
      }
      open(payload);
  ...
  // Namespaces below back the documented `pluginContext.dom/time/formatting/shared` surface.
  // MMM-Webuntis.js forwards them verbatim, so the grouping is curated here only - see
  // docs/PLUGINS.md. `root.util` keeps exposing everything for existing callers.
  root.time = {
    DEFAULT_TIMEZONE,
