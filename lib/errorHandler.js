 ...
 * @returns {Object|null} Warning object { message, category } or null if no conversion available
 *
 * @example
 ...
 * )
 * // Returns: { message: 'Authentication failed...', category: 'permanent' }
 */
function convertRestErrorToWarning(error, context = {}) {
  ...
  if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT' || msg.includes('timeout')) {
    return {
      message: `Cannot connect to WebUntis server "${server}". Check server name and network connection.`,
      category: 'temporary',
    };
  }
  ...
  if (status === 503 || msg.includes('503')) {
    return {
      message: `WebUntis API temporarily unavailable (HTTP 503). Retrying on next fetch...`,
      category: 'temporary',
    };
  }

  // Generic network error (TEMPORARY)
  if (!error.response) {
    return {
      message: `Network error connecting to WebUntis: ${error.message || 'Unknown error'}`,
      category: 'temporary',
    };
  }

  // Authentication errors (PERMANENT after retry)
  if (status === 401 || status === 403 || msg.includes('401') || msg.includes('403')) {
    return {
      message: `Authentication failed for "${studentTitle}": Invalid credentials or insufficient permissions.`,
      category: 'permanent',
    };
  }

  // School not found (PERMANENT)
  if (msg.includes('school') || msg.includes('not found')) {
    return {
      message: `School "${school}" not found or invalid credentials. Check school name and spelling.`,
      category: 'permanent',
    };
  }
  ...
  if (status && status >= 400 && status < 500) {
    return {
      message: `HTTP ${status} error for "${studentTitle}": ${error.message || 'Client error'}`,
      category: 'permanent',
    };
  }
  ...
  if (status && status >= 500) {
    return {
      message: `Server error (HTTP ${status}): ${error.message || 'Server error'}`,
      category: 'temporary',
    };
  }
