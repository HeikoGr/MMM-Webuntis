...
function collectPluginDefaultConfig(pluginRecords = []) {
  return pluginRecords.reduce((acc, pluginRecord) => {
    const defaults = typeof pluginRecord.instance?.getDefaultConfig === 'function' ? pluginRecord.instance.getDefaultConfig() : null;
    if (defaults && typeof defaults === 'object' && !Array.isArray(defaults)) {
      acc[pluginRecord.id] = defaults;
    }
    return acc;
  }, {});
}

function validatePluginConfigs(pluginRecords = [], pluginConfigs = {}, helpers = {}) {
  const issues = [];

  for (const pluginRecord of pluginRecords) {
    if (typeof pluginRecord.instance?.validateConfig !== 'function') continue;
    const pluginConfig = pluginConfigs?.[pluginRecord.id] || {};
    const result = pluginRecord.instance.validateConfig(pluginConfig, helpers);
