    ...
    if (Array.isArray(config.excludeLessons)) {
      config.excludeLessons.forEach((entry, idx) => {
        if (!buildMatcher(entry)) {
          warnings.push(`${prefix}excludeLessons[${idx}]: must be a non-empty string or a valid "/regex/" – entry ignored`);
        }
      });
    } else {
...
function filterExcludedHomework(homework, excludeLessons) {
  return filterExcludedItems(homework, excludeLessons, (hw) => [hw?.subject?.name, hw?.subject?.longname].filter(Boolean).map(String));
}
