...
const schemas = {
  lesson: {
    ...
    changedFields: {
      from: 'changedFields',
      transform: (v) => (Array.isArray(v) ? v.filter((field) => typeof field === 'string' && field.trim().length > 0) : []),
      default: [],
    },
  },
  dayNotice: {
...
function buildStudentErrorPayload({
  ...
  payload.state.warningMeta = buildWarningMetaList(warnings, warningMetaByMessage, warningFallbackMeta || DEFAULT_WARNING_META);
  return payload;
}
