...
const { parseHHMMStringToInteger, sanitizeRichText, compactTimegrid } = require("../webuntis/dataOrchestration");
const { cleanupOldDumps } = require("../webuntis/debugDumpUtils");
const { DEFAULT_WARNING_META, buildWarningMetaList, createWarningCollector } = require("../warningUtils");
const {
  buildCustomLessons,
  filterExcludedExams,
  ...
  resolveListOption,
} = require("./lessonAdjustments");

function formatSubject(subject) {
  if (!subject) return null;
  if (typeof subject === "object" && !Array.isArray(subject)) {
    return { name: subject.name || "", longname: subject.longname || "" };
  }
  if (Array.isArray(subject) && subject[0]) {
    return { name: subject[0].name || "", longname: subject[0].longname || "" };
  }
  return null;
}
...
const schemas = {
  lesson: {
    ...
    substitutionText: { from: "substitutionText", default: "" },
    lessonText: { from: "lessonText", default: "" },
    status: { from: "status", default: "REGULAR" },
    displayIcons: {
      from: "displayIcons",
      transform: (v) =>
        Array.isArray(v) ? v.filter((icon) => typeof icon === "string" && icon.trim().length > 0) : [],
      default: [],
    },
    id: { from: "id", default: null },
    previousTeachers: {
      from: "previousTeachers",
      transform: (v) => (Array.isArray(v) && v[0] ? [{ name: v[0].name, longname: v[0].longname }] : []),
      default: [],
    },
    previousSubjects: {
      from: "previousSubjects",
      transform: (v) => (Array.isArray(v) && v[0] ? [{ name: v[0].name, longname: v[0].longname }] : []),
      default: [],
    },
    previousRooms: {
    ...
    changedFields: {
      from: "changedFields",
      transform: (v) =>
        Array.isArray(v) ? v.filter((field) => typeof field === "string" && field.trim().length > 0) : [],
      default: [],
    },
  },
  dayNotice: {
    date: { from: "date", transform: (v) => parseInt(String(v).replace(/\D/g, ""), 10) || 0 },
    kind: { from: "kind", default: "" },
    status: { from: "status", default: "" },
  },
  exam: {
    examDate: {
      from: "examDate",
    ...
    name: { from: "name", transform: (v) => sanitizeRichText(v, false), default: "" },
    subject: { from: "subject", transform: (v) => sanitizeRichText(v, false), default: "" },
    teachers: { from: "teachers", transform: (v) => (Array.isArray(v) ? v.slice(0, 2) : []), default: [] },
    text: { from: "text", transform: (v) => sanitizeRichText(v, true), default: "" },
  },
  homework: {
    id: { from: "id", default: null },
    ...
    text: {
      ...
      transform: (v) => sanitizeRichText(v, true),
      default: "",
    },
    remark: { from: "remark", transform: (v) => sanitizeRichText(v, false), default: "" },
    subject: {
      from: "subject",
  ...
  absence: {
    ...
    reason: {
      ...
      transform: (v) => sanitizeRichText(v, false),
      default: "",
    },
...
const COLLECTION_ENDPOINTS = Object.freeze({
  lessons: "timetable",
  exams: "exams",
  homework: "homework",
  absences: "absences",
...
/**
 * Per-collection reliability state for `state.collections`.
 *
 * - `disabled`: the collection is not fetched for this student (no plugin needs it)
 * - `ok`: the last call for this collection succeeded (2xx, structurally valid)
 * - `unavailable`: the last call failed, was skipped by the circuit breaker / 24h rule, never
 *   ran (no REST target) or the whole fetch failed before reaching the endpoint
 *
 * `lastSuccessAt` survives failures so the frontend can tell how stale preserved data is.
 *
 * @param {Object} params
 * @param {Object} params.fetchFlags - Fetch flags
 * @param {Object} params.apiRecords - collection -> { status, recordedAt, failureCount, lastSuccessAt }
 * @param {boolean} [params.forceUnavailable=false] - Mark every enabled collection unavailable
 * @returns {Object} collection -> { status, httpStatus, lastSuccessAt }
 */
function buildCollectionStates({ fetchFlags = {}, apiRecords = {}, forceUnavailable = false }) {
  const enabled = {
    lessons: Boolean(fetchFlags.fetchTimetable),
    exams: Boolean(fetchFlags.fetchExams),
    homework: Boolean(fetchFlags.fetchHomeworks),
    absences: Boolean(fetchFlags.fetchAbsences),
    messages: Boolean(fetchFlags.fetchMessagesOfDay),
  };

  const result = {};
  for (const [collection, endpoint] of Object.entries(COLLECTION_ENDPOINTS)) {
    const record = apiRecords?.[endpoint] || apiRecords?.[collection] || null;
    const httpStatus = Number.isFinite(Number(record?.status)) ? Number(record.status) : null;
    const succeeded = httpStatus !== null && httpStatus >= 200 && httpStatus < 300;
    let status = "unavailable";
    if (!enabled[collection]) status = "disabled";
    else if (!forceUnavailable && succeeded) status = "ok";
...
function compactPayloadData({ grid, timetable, rawExams, hwResult, rawAbsences, rawMessagesOfDay, fetchFlags }) {
  ...
  return {
    ...
    compactDayNotices: compactArray(timetable?.dayNotices, schemas.dayNotice),
...
function buildBasePayload({
  ...
  return {
    ...
    data: {
      ...
      dayNotices: compacted.compactDayNotices,
    ...
    state: {
      ...
      collections: buildCollectionStates({ fetchFlags, apiRecords, forceUnavailable }),
      warnings: [],
      warningMeta: [],
    },
  };
...
function collectWarnings({
  ...
  // Diagnoses raised by the fetch itself rather than by config validation - currently the
  // "no REST targets" hint from webuntisClient._logEmptyTargets.
  if (Array.isArray(configWarnings)) {
    configWarnings.forEach((warning) => {
      warningCollector.addWarning(warning, { kind: "config", severity: "warning" });
    });
  }

  const timetableApiStatus = Number(apiStatus?.timetable);
  const hasSuccessfulTimetableStatus =
    Number.isFinite(timetableApiStatus) && timetableApiStatus >= 200 && timetableApiStatus < 300;
...
function createOrderedDumpPayload(safePayload) {
  ...
  if (safePayload.contractVersion !== undefined) orderedPayload.contractVersion = safePayload.contractVersion;
  ...
  Object.keys(safePayload).forEach((k) => {
    if (k === "contractVersion" || k === "meta" || k === "context") return;
...
function writeDebugDump({ payload, moduleConfig, student, mmLog }) {
  tryOrNull(
    () => {
      ...
      cleanupOldDumps(dumpDir, 25, (f) => f.endsWith("_api.json") && !f.startsWith("raw_api_"));
...
// dateRanges.timetable.end is exclusive (WebUntis API semantics); return the last included day.
function lastDayOfExclusiveRange(end) {
...
// Apply excludeLessons/addLessons (student level overrides module level) to the compacted lessons.
function applyLessonAdjustments({ lessons, student, moduleConfig, dateRanges, compactHolidays, shouldFetchTimetable }) {
  const excludeLessons = resolveListOption(student, moduleConfig, "excludeLessons");
  const addLessons = resolveListOption(student, moduleConfig, "addLessons");
  if (excludeLessons.length === 0 && addLessons.length === 0) return lessons;

  const remaining = filterExcludedLessons(lessons, excludeLessons);
  if (!shouldFetchTimetable || addLessons.length === 0) return remaining;

  const customLessons = compactArray(
    buildCustomLessons(addLessons, {
      startYmd: toYmd(dateRanges?.timetable?.start),
      endYmd: lastDayOfExclusiveRange(dateRanges?.timetable?.end),
      holidays: compactHolidays,
    }),
    schemas.lesson,
  );
  if (customLessons.length === 0) return remaining;

  return [...remaining, ...customLessons].sort((a, b) => a.date - b.date || (a.startTime ?? 0) - (b.startTime ?? 0));
}
...
function buildGotDataPayload(params) {
  ...
  compacted.compactTimetable = applyLessonAdjustments({
    lessons: compacted.compactTimetable,
    student,
    moduleConfig,
    dateRanges,
    compactHolidays,
    shouldFetchTimetable: compacted.shouldFetchTimetable,
  });
  // Homework and exams of excluded subjects are hidden as well.
  const excludeLessons = resolveListOption(student, moduleConfig, "excludeLessons");
  compacted.compactHomeworks = filterExcludedHomework(compacted.compactHomeworks, excludeLessons);
  compacted.compactExams = filterExcludedExams(compacted.compactExams, excludeLessons);
...
/**
 * DATA_UPDATE payload for a student whose fetch did not produce data (auth failure, fetch
 * exception): same contract shape as a regular payload, empty collections, warnings attached.
 *
 * @param {Object} params
 * @param {string} params.identifier - Module identifier
 * @param {string} params.sessionId - Session id
 * @param {Object} params.student - Student config
 * @param {Object} params.config - Module config
 * @param {Object} params.fetchFlags - Fetch flags (state.fetch)
 * @param {Object} params.apiStatus - state.api snapshot (null values when unknown)
 * @param {string[]} params.warnings - Warning messages
 * @param {Map} [params.warningMetaByMessage] - message -> meta
 * @param {Object} [params.warningFallbackMeta] - Meta for messages without an entry
 * @returns {Object} DATA_UPDATE payload
 */
function buildStudentErrorPayload({
  identifier,
  sessionId,
  student,
  config,
  ...
  warningFallbackMeta = DEFAULT_WARNING_META,
}) {
  const payload = buildBasePayload({
    student: student || {},
    moduleConfig: config,
    compacted: {
      ...
      compactHomeworks: [],
      compactAbsences: [],
      compactMessagesOfDay: [],
      shouldFetchTimetable: Boolean(fetchFlags.fetchTimetable),
      shouldFetchHomeworks: Boolean(fetchFlags.fetchHomeworks),
      shouldFetchAbsences: Boolean(fetchFlags.fetchAbsences),
      shouldFetchMessagesOfDay: Boolean(fetchFlags.fetchMessagesOfDay),
    },
    compactHolidays: [],
    activeHoliday: null,
    todayYmd: null,
  ...
  payload.context.student.title = title;
  payload.id = identifier;
  payload.sessionId = sessionId;
  payload.state.warnings = warnings.slice();
  payload.state.warningMeta = buildWarningMetaList(
    warnings,
...
function mapBundleToMmmPayload(bundle, deps = {}) {
  ...
  const { dateRanges, todayYmd, activeHoliday, fetchFlags, apiStatus, apiRecords, data, configWarnings } = coreData;
...
module.exports = {
  buildCollectionStates,
  buildStudentErrorPayload,
