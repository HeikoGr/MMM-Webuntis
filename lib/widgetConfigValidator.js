...
function buildUpperRangeWarning(path, value) {
  return `${path} exceeds the recommended range. Value: ${value}`;
}

function buildLowerRangeWarning(path, value) {
  return `${path} is below the recommended range. Value: ${value}`;
}

/**
 * Common validation helper: checks if value exists and is finite
 * @param {*} value - Value to validate
 * @param {Array} warnings - Accumulated warning messages
 * @param {string} path - Full field path for error messages
 * @returns {boolean} True if validation should continue, false if value is invalid/missing
 */
function validateFieldPrerequisites(value, warnings, path) {
  // Skip undefined/null (optional fields not configured)
  if (value === undefined || value === null) return false;

  // Reject non-finite values (Infinity, NaN)
  if (!Number.isFinite(value)) {
    warnings.push(`${path} must be a number. Value: ${value}`);
    return false;
  }
 ...
 * @param {Array} warnings - Accumulated warning messages
 * @param {string} scope - Config path prefix (e.g., 'widget.grid')
 * @param {Object} config - Configuration object containing the field
 * @param {string} key - Field name to validate
 * @param {Object} options - Customization options
 * @param {Function} [options.upperCondition] - Optional condition to check for upper range warning
 * @param {Function} [options.upperMessage] - Custom warning message callback for upper range
 */
function validateNonNegativeField(warnings, scope, config, key, options = {}) {
  const value = config?.[key];
  const path = buildFieldPath(scope, key);
  if (!validateFieldPrerequisites(value, warnings, path)) return;
  ...
  if (typeof options.upperCondition === 'function' && options.upperCondition(value)) {
    if (typeof options.upperMessage === 'function') {
      warnings.push(options.upperMessage(value, path));
    } else {
      warnings.push(buildUpperRangeWarning(path, value));
    }
  }
}

/**
 * Validate a numeric field has a minimum value
 * @param {Array} warnings - Accumulated warning messages
 * @param {string} scope - Config path prefix (e.g., 'widget.grid')
 * @param {Object} config - Configuration object containing the field
 * @param {string} key - Field name to validate
 * @param {number} minimum - Minimum allowed value (inclusive)
 * @param {Object} options - Customization options
 * @param {Function} [options.lowerMessage] - Custom warning message callback(value, path)
 */
function validateMinimumField(warnings, scope, config, key, minimum, options = {}) {
  ...
  // Validate minimum parameter itself
...
function validatePositiveCoercedField(warnings, scope, config, key, options = {}) {
  ...
  if (typeof options.lowerCondition === 'function' && options.lowerCondition(value)) {
    if (typeof options.lowerMessage === 'function') {
      ...
      warnings.push(buildLowerRangeWarning(path, value));
    }
    return;
  }
