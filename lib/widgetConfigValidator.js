...
function validateGridConfig(gridConfig) {
  ...
  // maxLessons validation
  if (Number.isFinite(gridConfig.maxLessons)) {
    if (gridConfig.maxLessons < 1) {
      warnings.push(`grid.maxLessons must be at least 1. Value: ${gridConfig.maxLessons}`);
    } else if (gridConfig.maxLessons > 20) {
...
function validateAbsencesConfig(absencesConfig) {
  ...
  // futureDays validation
  if (Number.isFinite(absencesConfig.futureDays)) {
    if (absencesConfig.futureDays < 0) {
      warnings.push(`absences.futureDays cannot be negative. Value: ${absencesConfig.futureDays}`);
    } else if (absencesConfig.futureDays > 90) {
      warnings.push(`absences.futureDays is very large (${absencesConfig.futureDays}). Typical values: 7-30.`);
    }
  }
