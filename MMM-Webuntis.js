Module.register("MMM-Webuntis", {
   ...
   */
    const daysToShow = studentConfig.daysToShow && studentConfig.daysToShow > 0
      ? parseInt(studentConfig.daysToShow)
      : 1;
    ...
    if (Array.isArray(timeUnits) && timeUnits.length > 0) {
      timeUnits.forEach((u) => {
        if (u.startMin !== undefined && u.startMin !== null) {
          allStart = Math.min(allStart, u.startMin);
        }
        if (u.endMin !== undefined && u.endMin !== null) {
    ...
    if (Array.isArray(timeUnits) && timeUnits.length > 0) {
      for (const u of timeUnits) {
        ...
        lab.style.color = "#666";
        lab.innerText = `${u.name} Std.\n ${String(u.startTime).padStart(4, "0")
          .replace(/(\d{2})(\d{2})/, "$1:$2")}`;
    ...
    for (let d = 0; d < totalDisplayDays; d++) {
      ...
      const dayLessons = this.preprocessedByStudent && this.preprocessedByStudent[studentTitle] && this.preprocessedByStudent[studentTitle].groupedByDate && this.preprocessedByStudent[studentTitle].groupedByDate[dateStr]
      ...
      for (let i = 0; i < dayLessons.length; i++) {
        ...
        if (firstId !== null && firstId !== undefined) {
          curr.lessonIds.push(String(firstId));
        }
        ...
        while (j < dayLessons.length) {
          ...
          const currEndMin = curr.endMin !== undefined && curr.endMin !== null
            ? curr.endMin
            : null;
          const candStartMin = cand.startMin !== undefined && cand.startMin !== null
            ? cand.startMin
            : null;
          const candEndMin = cand.endMin !== undefined && cand.endMin !== null
            ? cand.endMin
            : null;
          ...
          curr.endMin = candEndMin !== null
            ? candEndMin
            : candStartMin + 45;
          curr.substText = curr.substText || "";
          curr.text = curr.text || "";
          if (cand.substText && !curr.substText.includes(cand.substText)) {
            curr.substText += `\n${cand.substText}`;
          }
          if (cand.text && !curr.text.includes(cand.text)) {
            curr.text += `\n${cand.text}`;
          }
          // collect lesson ids
        ...
        if ((!curr.lessonId || curr.lessonId === null) && curr.lessonIds && curr.lessonIds.length > 0) {
          curr.lessonId = curr.lessonIds[0];
        }
        ...
        if (curr.startMin === undefined || curr.startMin === null) {
          ...
          if (this.config.logLevel === "debug") {
            this._log("warn", "Merged lesson missing startMin; backend should provide numeric startMin/endMin", curr.lessonId
      ...
      try {
        if (Array.isArray(timeUnits) && timeUnits.length > 0) {
          for (const u of timeUnits) {
            ...
            if (u.endMin < allStart || u.endMin > allEnd) {
              continue;
            }
      ...
      for (let idx = 0; idx < mergedLessons.length; idx++) {
        ...
        const makeInner = (lsn) => {
          ...
          const subst = lsn.substText
            ? `<br><span class='xsmall dimmed'>${lsn.substText.replace(/\n/g, "<br>")}</span>`
            : "";
          const txt = lsn.text
            ? `<br><span class='xsmall dimmed'>${lsn.text.replace(/\n/g, "<br>")}</span>`
            : "";
        ...
        if (homeworks && Array.isArray(homeworks)) {
          const hwMatch = homeworks.some((hw) => {
            ...
            const lessonIds = lesson.lessonIds && Array.isArray(lesson.lessonIds)
              ? lesson.lessonIds
              : lesson.lessonId
                ? [String(lesson.lessonId)]
                : [];
            const lessonIdMatch = hwLessonId && lessonIds.length > 0
              ? lessonIds.includes(String(hwLessonId))
              : false;
          ...
          if (hwMatch) {
            ...
            icon.innerHTML = "📘";
            if (leftCell && leftCell.innerHTML) {
              leftCell.appendChild(icon.cloneNode(true));
            } else if (rightCell && rightCell.innerHTML) {
              rightCell.appendChild(icon);
            }
          }
        }
    ...
    try {
      ...
      if (this.config && this.config.logLevel === "debug") {
        this._log("warn", "now-line updater scheduling failed", e);
      }
    }
  ...
  /* Cleanup when module is suspended/hidden to free memory */
  suspend () {
    if (this._fetchTimer) {
  ...
  /* Resume timers when module is shown again */
  resume () {
    // Restart now-line updater
    if (!this._nowLineTimer) {
      try {
        this._nowLineTimer = setInterval(() => {
          try {
    ...
    try {
      ...
      inners.forEach((inner) => {
        try {
          ...
          if (!nl || allS === undefined || allE === undefined || h === undefined) {
            return;
          }
  ...
  getDom () {
    ...
    for (const studentTitle of sortedStudentTitles) {
      ...
      const todayLessons = this.todayLessonsByStudent[studentTitle];
      const timeUnits = this.timeUnitsByStudent[studentTitle];

      const homeworks = this.homeworksByStudent && this.homeworksByStudent[studentTitle]
  ...
  notificationReceived (notification, payload) {
    switch (notification) {
      case "DOM_OBJECTS_CREATED":
        // Prevent multiple timers - only create if not already running
        if (!this._fetchTimer) {
    ...
    if (notification === "GOT_DATA") {
      ...
      if (this.config.logLevel === "debug") {
        const cLessons = Array.isArray(payload.lessons)
          ...
          : 0;
        const cExams = Array.isArray(payload.exams)
          ? payload.exams.length
          : 0;
